...
\title{Figurer og tabeller for årsrapport NORGAST 2024 - Lever og pankreas}
\author{NORGAST}
...
DG <- read.csv2(system.file("extdata/dg2024.csv", package = "norgast"),
                fileEncoding = "UTF-8") |>
  dplyr::filter(Op_gr == 5) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_lever <- DG$Sykehusnavn[DG$DG < dgterskel]

DG <- read.csv2(system.file("extdata/dg2024.csv", package = "norgast"),
                fileEncoding = "UTF-8") |>
  dplyr::filter(Op_gr == 6) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_whipple <- DG$Sykehusnavn[DG$DG < dgterskel]

DG <- read.csv2(system.file("extdata/dg2024.csv", package = "norgast"),
                fileEncoding = "UTF-8") |>
  dplyr::filter(Op_gr == 7) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_pankreas <- DG$Sykehusnavn[DG$DG < dgterskel]
...
annet_format_ut <- T
ut_format <- 'svg'
figfolder <- "C:/Users/kth200/OneDrive - Helse Nord RHF/Dokumenter/regdata/norgast/aarsrapp2024/pankreas_lever/"
