...
dgterskel <- 70
DG <- read.csv2('~/mydata/norgast/DG_Sykehus/Lever_sh.csv') %>%
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) %>%
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_lever <- DG$Sykehusnavn[DG$DG_norgast < dgterskel]

DG <- read.csv2('~/mydata/norgast/DG_Sykehus/Whipple_sh.csv') %>%
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) %>%
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_whipple <- DG$Sykehusnavn[DG$DG_norgast < dgterskel]

DG <- read.csv2('~/mydata/norgast/DG_Sykehus/Distal_pankreas_sh.csv') %>%
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) %>%
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_pankreas <- DG$Sykehusnavn[DG$DG_norgast < dgterskel]

# DG <- read.csv2('~/mydata/norgast/DG_Sykehus/Andre_pankreas_sh.csv') %>%
#   merge(sh_norgast, by = "AvdRESH", all.x = TRUE) %>%
#   dplyr::filter(!is.na(Sykehusnavn))
# graaUt_pankreas <- DG$Sykehusnavn[DG$DG_norgast < 70]
