...
tab_tilbakelegg <- bind_rows(
  ...
  bind_rows(
    data.frame("Tilgang_ny"="\\textbf{Ileostomi: laparatomi}",
               "N"="", "reop_rate"=NA, "anastomoselekk"=NA,
               "dyp_infek"=NA, "bloedning"=NA, "saarrupt"=NA,
               "annet"=NA, "ingen"=NA)
  ) |>
  ...
  bind_rows(Tabell_tilbakelegg_kolo_lokal |>
              mutate(Tilgang_ny = paste0("\\quad ", Tilgang_ny))) |>
  bind_rows(
    data.frame("Tilgang_ny"="\\textbf{Kolostomi: laparatomi}",
               "N"="", "reop_rate"=NA, "anastomoselekk"=NA,
               "dyp_infek"=NA, "bloedning"=NA, "saarrupt"=NA,
               "annet"=NA, "ingen"=NA)
  ) |>
...
RegData <- RegData |>
  mutate(
    tilgang_tilbakelegging = case_when(
      ncsp_lowercase %in% c("jfg00", "jfg10", "jfg20", "jfg30") ~ "Lokal tilgang",
      ncsp_lowercase %in%
        c("jfg23", "jfg26", "jfg29", "jfg33", "jfg36") ~ "Laparotomi",
      .default = NA
      ) |> as.factor(),
