...
\title{Figurer og tabeller for årsrapport NORGAST 2025 - Kolon og rektum}
\author{NORGAST}
...
# RegData$Sykehusnavn[
#   RegData$Sykehusnavn == "Sjukehuset Nordmøre og Romsdal"] <-
#   "Nordmøre og Romsdal"
...
RegDataAll <- RegDataAll %>%
  dplyr::mutate(
    Tilgang_utvidet2 = Tilgang_utvidet,
...
DG <- readr::read_csv2(
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/DGA Norgast_utkast2/Sykehus/Kolon_sh.csv", locale = locale(encoding = "WINDOWS-1252")) |>
...
graaUt_kolon <- DG$Sykehusnavn[DG$DG_norgast < dgterskel]
# graaUt_kolon <- ""
DG <- readr::read_csv2(
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/DGA Norgast_utkast2/Sykehus/Rektum_sh.csv", locale = locale(encoding = "WINDOWS-1252")) |>
  dplyr::mutate(AvdRESH = substr(AvdRESH, 1, 7),
                AvdRESH = ifelse(AvdRESH == 4204126, 4204084, AvdRESH)) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_rektum <- DG$Sykehusnavn[DG$DG_norgast < dgterskel]
# graaUt_rektum <- ""
...
figfolder <- "C:/Users/kth200/regdata/norgast/aarsrapp2025/kolon/"
...
\begin{small}
\centering
<<'Tabell:ReoperasjonerKolon malign', results='asis', echo=FALSE, eval=T, warning=FALSE>>=
print(
  xtable::xtable(Tabell_malign, digits=c(0,0,0,rep(1, ncol(Tabell_malign)-2)),
                 align=c('l', 'l', rep('r', ncol(Tabell_malign)-1))),
  floating=FALSE,
  hline.after=NULL,
  add.to.row=list(
    pos=list(-1,0,0,0,0,0, nrow(Tabell_malign)),
...
@
\captionof{table}{Kolonreseksjoner. Elektive operasjoner, malign diagnose. Reoperasjoner \Sexpr{rap_aar} etter robotassistert og ikke-robotassistert laparoskopi. ITT = Intention to treat.}
\end{small}
\end{minipage}
\end{table}
...
\begin{small}
\centering
<<'Tabell:ReoperasjonerKolon benign', results='asis', echo=FALSE, eval=T, warning=FALSE>>=
print(
  xtable::xtable(Tabell_benign, digits=c(0,0,0,rep(1, ncol(Tabell_benign)-2)),
                 align=c('l', 'l', rep('r', ncol(Tabell_benign)-1))),
  floating=FALSE,
  hline.after=NULL,
  add.to.row=list(
    pos=list(-1,0,0,0,0,0, nrow(Tabell_benign)),
    command=c('\\toprule\n', '& & & \\multicolumn{4}{c}{Reoperasjonsrate for
              ulike årsaker (\\%)} \\\\\n', '\\cline{4-9} \n',
              '& & Reoperasjons- & Anastomose- & Dyp &&&&  \\\\\n',
              'Tilgang & N & rate (\\%) & lekkasje\\footnote[2]{Dette er av alle
  ...
  sanitize.text.function = function(x){x})
@
\captionof{table}{Kolonreseksjoner. Elektive operasjoner, benign diagnose. Reoperasjoner \Sexpr{rap_aar} etter robotassistert og ikke-robotassistert laparoskopi. ITT = Intention to treat.}
\end{small}
\end{minipage}
\end{table}
...
NorgastFigAndelStabelGrVar(
  RegData, valgtVar='AccordionGrad_drenasje',
  op_gruppe=op_gruppe,
  hastegrad_hybrid=1, outfile=outfile, Ngrense=10,
...
norgastIndikator_rapporteket(
  RegDataAll, valgtVar = valgtVar,
  outfile=outfile, tittel=tittel, width=width,
  height=height, decreasing=T, terskel=terskel,
  minstekrav = NA, maal = 12,
  skriftStr=skriftStr, maalretn='lav',
  lavDG = graaUt_kolon, lavDGtekst = dg_tekst,
  pktStr=pktStr, legPlass='topright',
  minstekravTxt=minstekravTxt, maalTxt=maalTxt,
  graaUt=graaUt, whoEcog= c('0', '1'),
...
outfile <- paste0(figfolder, outfile)
valgtVar <- 'konv_rate'
tmptall <- norgastIndikator_rapporteket(
  RegDataAll, valgtVar = valgtVar,
  outfile=outfile, tittel=tittel, width=width,
  height=height, decreasing=T, terskel=terskel,
  minstekrav = 15, maal = 10,
  skriftStr=skriftStr, maalretn='lav',
  lavDG = graaUt_kolon, lavDGtekst = dg_tekst,
  pktStr=pktStr, legPlass='topright',
  minstekravTxt=minstekravTxt, maalTxt=maalTxt,
...
outfile <- paste0(figfolder, outfile)
utdata_kolon_anastomoselekk_refpas <-
  norgastIndikator_rapporteket(
    RegDataAll, valgtVar = valgtVar,
    outfile=outfile, tittel=tittel, width=width,
    height=height, decreasing=T, terskel=terskel,
...
utdata_kolon_mort <- norgastIndikator_rapporteket(
  ...
  legPlass=legPlass, minstekravTxt=minstekravTxt,
  maalTxt=maalTxt, graaUt=graaUt,
  lavDG = graaUt_kolon, lavDGtekst = dg_tekst,
  inkl_konf=inkl_konf, op_gruppe=op_gruppe,
  datoFra=datoFra, datoTil=datoTil,
  hastegrad_hybrid=99, malign=99,
  kun_ferdigstilte = F, hastegrad = 1,
  prikktall = FALSE, pst_kolonne = TRUE)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
...
utdata_kolon_mort <- norgastIndikator_rapporteket(
  ...
  datoFra=datoFra, datoTil=datoTil,
  hastegrad_hybrid=99, malign=99,
  kun_ferdigstilte = F, hastegrad = 2,
  prikktall = FALSE, pst_kolonne = TRUE)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
...
norgastIndikator_rapporteket(
  RegDataAll, valgtVar = valgtVar,
  outfile=outfile, tittel=tittel, width=width,
  height=height, decreasing=F, terskel=terskel,
  minstekrav = NA, maal = NA,
  skriftStr=skriftStr, pktStr=pktStr,
  legPlass=legPlass, minstekravTxt=minstekravTxt,
  maalTxt=maalTxt, graaUt=graaUt,
  lavDG = graaUt_kolon, lavDGtekst = dg_tekst,
  inkl_konf=inkl_konf, op_gruppe=op_gruppe,
  datoFra='2018-05-01', datoTil=datoTil,
  hastegrad_hybrid=99, malign=99,
  prikktall = FALSE, pst_kolonne = TRUE)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
    ...
    skriftStr=skriftStr, pktStr=pktStr,
    legPlass=legPlass, minstekravTxt=minstekravTxt,
    maalTxt=maalTxt, graaUt=graaUt, lavDG = graaUt_kolon,
    lavDGtekst = dg_tekst,
    inkl_konf=inkl_konf, op_gruppe=op_gruppe,
    datoFra='2018-05-01', datoTil=datoTil,
    hastegrad_hybrid=99, malign=99,
...
outfile <- paste0(figfolder, outfile)
norgast::norgastAndelGruppert1Gr(
  valgtVar="Anastomoselekkasje", outfile = outfile,
  Grvar1 = "Robot",
  ...
  malign=1, ytxt = "",
  hastegrad = 1, whoEcog = 0:1, inkl_konf = TRUE, tilgang = c(2,3))
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgast::norgastAndelGruppert1Gr(
    valgtVar="Anastomoselekkasje", outfile = outfile,
...
outfile <- paste0(figfolder, outfile)
norgast::norgastAndelGruppert1Gr(
  valgtVar="konvertert", outfile = outfile,
  Grvar1 = "Robot",
...
<<'ReoperasjonerRektum ny 2025', results='asis', echo=FALSE, eval=T, warning=FALSE>>=
Tabell_rektum <- RegData %>%
  # filter(Malign == 0) |>
  filter(Hastegrad_hybrid==1) %>%
  filter(ReLapNarkose %in% c(0, 1)) %>%
  filter(Op_gr==2) %>%
...
\begin{small}
\centering
<<'Tabell:ReoperasjonerRektum ny 2025', results='asis', echo=FALSE, eval=T, warning=FALSE>>=
print(
  xtable::xtable(
    Tabell_rektum, digits=c(0,0,0,rep(1, ncol(Tabell_rektum)-2)),
    align=c('l', 'l', rep('r', ncol(Tabell_rektum)-1))),
  floating=FALSE,
  hline.after=NULL,
  add.to.row=list(
    pos=list(-1,0,0,0,0,0, nrow(Tabell_malign)),
...
NorgastFigAndelStabelGrVar(
  ...
  lavDGtekst = dg_tekst, icd_kode = "C20")
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  NorgastFigAndelStabelGrVar(
    RegData, valgtVar='AvstandAnalVerge_kat',
...
norgastIndikator_rapporteket(
  ...
  height=height, decreasing=decreasing,
  terskel=terskel, minstekrav = 60, maal = 70,
  skriftStr=skriftStr, lavDG = graaUt_rektum,
  lavDGtekst = dg_tekst, pktStr=pktStr,
  legPlass=legPlass, minstekravTxt=minstekravTxt,
  maalTxt=maalTxt, graaUt=graaUt,
  whoEcog = 0:1, inkl_konf=inkl_konf,
  op_gruppe=op_gruppe, datoFra=datoFra,
...
norgastIndikator_rapporteket(
  ...
  minstekrav = 7, maal = 5,
  skriftStr=skriftStr, maalretn='lav',
  lavDG = graaUt_rektum, lavDGtekst = dg_tekst,
  pktStr=pktStr, legPlass='topright',
  minstekravTxt=minstekravTxt, maalTxt=maalTxt,
  graaUt=graaUt, whoEcog = 0:1,
  inkl_konf=inkl_konf, op_gruppe=op_gruppe,
...
outfile <- 'Anastomoselekkasje_rekt_malign_ecog_0_1_lap.pdf'
outfile <- paste0(figfolder, outfile)
norgastIndikator_rapporteket(
  RegDataAll, valgtVar = valgtVar,
  outfile=outfile, tittel=tittel, width=width,
  ...
  inkl_konf=inkl_konf, op_gruppe=op_gruppe,
  datoFra=datoFra, datoTil=datoTil,
  hastegrad_hybrid=1, malign=1, tilgang = 2:3,
  prikktall = FALSE, pst_kolonne = TRUE)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
    ...
    terskel=terskel, minstekrav = 7, maal = 5,
    skriftStr=skriftStr, maalretn='lav',
    lavDG = graaUt_rektum, lavDGtekst = dg_tekst,
    pktStr=pktStr, legPlass='topright',
    minstekravTxt=minstekravTxt, maalTxt=maalTxt,
...
norgastIndikator_gruppert(
  ...
  prikktall = FALSE, Grvar2 = "NyStomi_faktor")
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_gruppert(
    RegDataAll, valgtVar = valgtVar,
...
norgastIndikator_gruppert(
  RegDataAll, valgtVar = valgtVar,
  outfile=outfile, tittel=tittel, width=width,
  height=1100, decreasing=T, terskel=terskel,
  minstekrav = NA, maal = NA,
  skriftStr=skriftStr, lavDG = graaUt_rektum,
  lavDGtekst = dg_tekst,
  pktStr=pktStr, legPlass=legPlass,
  ...
  op_gruppe=op_gruppe, datoFra=datoFra,
  datoTil=datoTil, hastegrad_hybrid=hastegrad_hybrid,
  malign=1, ny_anastomose=1,
  kun_ferdigstilte = T, prikktall = FALSE,
  Grvar2 = "AvstandAnalVerge_kat2", mrom = 0.7)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_gruppert(
    RegDataAll, valgtVar = valgtVar,
    ...
    malign=1,ny_anastomose=1,
    kun_ferdigstilte = T, prikktall = FALSE,
    Grvar2 = "AvstandAnalVerge_kat2", mrom = 0.5)
}
...
valgtVar <- 'Anastomoselekkasje'
norgastIndikator_gruppert(
  RegData=aux, valgtVar = valgtVar,
  outfile=outfile, tittel=tittel, width=width,
  height=900, decreasing=T, terskel=terskel,
  minstekrav = NA, maal = NA,
  skriftStr=.7, lavDG = graaUt_rektum,
  lavDGtekst = dg_tekst, whoEcog = 0:1,
  pktStr=pktStr, legPlass=legPlass,
  minstekravTxt=minstekravTxt, maalTxt=maalTxt,
  graaUt=graaUt, inkl_konf=FALSE,
  op_gruppe=op_gruppe, datoFra=datoFra,
  datoTil=datoTil, hastegrad=1,
  malign=1, ny_anastomose=1,
  kun_ferdigstilte = T, prikktall = FALSE,
  Grvar2 = "robot_nystomi")
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_gruppert(
    RegData=aux, valgtVar = valgtVar,
...
valgtVar <- 'Anastomoselekkasje'
norgastIndikator_gruppert(
  RegData=aux, valgtVar = valgtVar, outfile=outfile,
...
outfile <- paste0(figfolder, outfile)
lekkdata <- norgast::norgastAndelGruppert1Gr(
  valgtVar="Anastomoselekkasje", outfile = outfile,
  Grvar1 = "Robot",
  tittel = "Andel anastomoselekkasjer, ny anastomose",
...
outfile <- paste0(figfolder, outfile)
norgast::norgastAndelGruppert2Gr(
  valgtVar="NyStomi", outfile = outfile,
...
norgastIndikator_rapporteket(
  ...
  lavDGtekst = dg_tekst, inkl_konf=inkl_konf,
  datoFra=datoFra, datoTil=datoTil, op_gruppe = 2,
  malign=malign, icd_kode = "C20") #
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
    RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
...
mapping <- read.csv2(
  ...
  fileEncoding = "Latin1") |>
  mutate(AvdRESH = ifelse(AvdRESH == 4204500, 4216808, AvdRESH))
aux <- RegDataAll |>
...
norgastIndikator_rapporteket(
  ...
  lavDGtekst = dg_tekst, inkl_konf=inkl_konf,
  datoFra=datoFra, datoTil=datoTil,
  malign=malign, icd_kode = "C20", op_gruppe = 2)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
    aux, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}bestraalt.pdf}
\caption{bestraalt.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}bestraalt_hf.pdf}
\caption{bestraalt\_hf.pdf}
\end{figure}

<<'beregn: bestraaling norgast og krg', results='asis', echo=FALSE, eval=T, warning=FALSE>>=

tab_krg <- read.csv2(
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/oppdatertesykehusnavn_NORGAST.csv",
  header = T, fileEncoding = "Latin1") |>
  dplyr::select(RESH, N_KRG2025, RateKRG2025) |>
  dplyr::mutate(ant_bestraalt = round(RateKRG2025*N_KRG2025/100)) |>
  janitor::adorn_totals() |>
  dplyr::mutate(
    RateKRG2025 = round(ant_bestraalt/N_KRG2025*100),
    RateKRG2025 = ifelse(is.nan(RateKRG2025), 0, RateKRG2025)) |>
  select(-ant_bestraalt)

Tabell_bestraaling <- RegData |>
  dplyr::filter(
    Op_gr == 2,
    sub("(\\w+).*", "\\1", RegData$Hoveddiagnose2) == "C20") |>
  dplyr::mutate(
    straaling = (KunStraaleterapi | KjemoRadioKombo)
  ) |>
  dplyr::summarise(ant_bestraalt = sum(straaling),
                   N_norgast = n(),
                   .by = c(AvdRESH, Sykehusnavn)) |>
  janitor::adorn_totals() |>
  mutate(rate_norgast = round(ant_bestraalt/N_norgast*100)) |>
  select(-ant_bestraalt)

krg_norgast <- merge(tab_krg, Tabell_bestraaling,
                     by.x = "RESH", by.y = "AvdRESH", all = T) |>
  dplyr::mutate(avvik = (rate_norgast - RateKRG2025)/RateKRG2025*100) |>
  dplyr::mutate(avvik15 = case_when(
    avvik > 15 ~ "+",
    avvik < -15 ~ "-",
    .default = ""
  )) |>
  dplyr::mutate(
    Sykehusnavn = ifelse(Sykehusnavn == "-", "Total", Sykehusnavn))

write.csv2(
  krg_norgast,
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/sammenlign_straaling_krg_norgast.csv",
...
\section{Tilbakelegging av stomi}


<<'beregn: Tilbakelegging av stomi', results='asis', echo=FALSE, eval=T, warning=FALSE>>=
Tabell_tilbakelegg_ileo_lokal <- RegData |>
  filter(ncsp_lowercase %in% c("jfg00", "jfg20")) |>
  filter(FerdigForlop == 1) |>
  norgast::lag_kompl_tabell()

Tabell_tilbakelegg_ileo_lap <- RegData |>
  # filter(Malign == 0) |>
  filter(ncsp_lowercase %in% c("jfg23", "jfg26", "jfg29")) |>
  filter(FerdigForlop == 1) |>
  norgast::lag_kompl_tabell()

Tabell_tilbakelegg_kolo_lokal <- RegData |>
  filter(ncsp_lowercase %in% c("jfg10", "jfg30")) |>
  filter(FerdigForlop == 1) |>
  norgast::lag_kompl_tabell()

Tabell_tilbakelegg_kolo_lap <- RegData |>
  # filter(Malign == 0) |>
  filter(ncsp_lowercase %in% c("jfg36", "jfg33")) |>
  filter(FerdigForlop == 1) |>
  norgast::lag_kompl_tabell()

tab_tilbakelegg <- bind_rows(
  data.frame("Tilgang_ny"="\\textbf{Ileostomi: lokal tilgang}",
             "N"="", "reop_rate"=NA, "anastomoselekk"=NA,
             "dyp_infek"=NA, "bloedning"=NA, "saarrupt"=NA,
             "annet"=NA, "ingen"=NA),
  Tabell_tilbakelegg_ileo_lokal |>
    mutate(Tilgang_ny = paste0("\\quad ", Tilgang_ny))) |>
  bind_rows(
    data.frame("Tilgang_ny"="\\textbf{Ileostomi: laparotomi/-skopi}",
               "N"="", "reop_rate"=NA, "anastomoselekk"=NA,
               "dyp_infek"=NA, "bloedning"=NA, "saarrupt"=NA,
               "annet"=NA, "ingen"=NA)
  ) |>
  bind_rows(Tabell_tilbakelegg_ileo_lap |>
              mutate(Tilgang_ny = paste0("\\quad ", Tilgang_ny))) |>
  bind_rows(
    data.frame("Tilgang_ny"="\\textbf{Kolostomi: lokal tilgang}",
               "N"="", "reop_rate"=NA, "anastomoselekk"=NA,
               "dyp_infek"=NA, "bloedning"=NA, "saarrupt"=NA,
               "annet"=NA, "ingen"=NA)
  ) |>
  bind_rows(Tabell_tilbakelegg_kolo_lokal |>
              mutate(Tilgang_ny = paste0("\\quad ", Tilgang_ny))) |>
  bind_rows(
    data.frame("Tilgang_ny"="\\textbf{Kolostomi: laparotomi/-skopi}",
               "N"="", "reop_rate"=NA, "anastomoselekk"=NA,
               "dyp_infek"=NA, "bloedning"=NA, "saarrupt"=NA,
               "annet"=NA, "ingen"=NA)
  ) |>
  bind_rows(Tabell_tilbakelegg_kolo_lap |>
              mutate(Tilgang_ny = paste0("\\quad ", Tilgang_ny)))
@
...
\begin{small}
\centering
<<'Tabell:Tilbakelegging', results='asis', echo=FALSE, eval=T, warning=FALSE>>=
print(
  xtable::xtable(
    tab_tilbakelegg, digits=c(0,0,0,rep(1, ncol(tab_tilbakelegg)-2)),
    align=c('l', 'l', rep('r', ncol(tab_tilbakelegg)-1))),
  floating=FALSE,
  hline.after=NULL,
  add.to.row=list(
    pos=list(-1,0,0,0,0,0, nrow(tab_tilbakelegg)),
    command=c(
      '\\toprule\n', '& & & \\multicolumn{4}{c}{Reoperasjonsrate for
              ulike årsaker (\\%)} \\\\\n', '\\cline{4-9} \n',
      '& & Reoperasjons- & Anastomose- & Dyp &&&&  \\\\\n',
      'Tilgang & N & rate (\\%) & lekkasje\\footnote[2]{Dette er av alle
              opererte, ikke alle under risiko.}
              & infeksjon & Blødning & Sårruptur
              & Annet & Ingen \\\\\n','\\midrule\n','\\bottomrule\n')),
  include.colnames=FALSE, include.rownames=FALSE,
  sanitize.text.function = function(x){x})
@
\captionof{table}{Tilbakelegging stomi. Reoperasjoner \Sexpr{rap_aar} etter robotassistert og ikke-robotassistert laparoskopi. ITT = Intention to treat.}
\end{small}
\end{minipage}
\end{table}
...
<<'fig_tilbakelegging', include=FALSE, echo=FALSE, eval=T, cache=FALSE>>=
op_gruppe <- 14
malign <- 99
elektiv <- 99
hastegrad_hybrid <- 99
...
valgtVar <- 'Anastomoselekkasje_alle'
outfile <- 'Anastomoselekkasje_tilbakelegging.pdf'
outfile <- paste0(figfolder, outfile)
utdata_kolon_anastomoselekk_refpas <-
  norgastIndikator_rapporteket(
    RegDataAll |> filter(FerdigForlop == 1), valgtVar = valgtVar,
    outfile=outfile, tittel=tittel, width=width,
    height=height, decreasing=T, terskel=terskel,
    # minstekrav = 6, maal = 4,
    skriftStr=skriftStr, maalretn='lav',
...
outfile <- 'Anastomoselekkasje_ileostomi.pdf'
outfile <- paste0(figfolder, outfile)
utdata_kolon_anastomoselekk_refpas <-
  norgastIndikator_rapporteket(
    RegDataAll |> filter(FerdigForlop == 1), valgtVar = valgtVar,
    ...
    pktStr=pktStr, legPlass='topright',
    minstekravTxt=minstekravTxt, maalTxt=maalTxt,
    graaUt=graaUt, ncsp = c("JFG00", "JFG20", "JFG23", "JFG26", "JFG29"),
    inkl_konf=inkl_konf, op_gruppe=op_gruppe,
    datoFra=datoFra, datoTil=datoTil,
    hastegrad_hybrid=hastegrad_hybrid, malign=malign,
    prikktall = FALSE, pst_kolonne = TRUE)
...
outfile <- 'Anastomoselekkasje_kolostomi.pdf'
outfile <- paste0(figfolder, outfile)
utdata_kolon_anastomoselekk_refpas <-
  norgastIndikator_rapporteket(
    RegDataAll |> filter(FerdigForlop == 1), valgtVar = valgtVar,
    ...
    pktStr=pktStr, legPlass='topright',
    minstekravTxt=minstekravTxt, maalTxt=maalTxt,
    graaUt=graaUt, ncsp = c("JFG10", "JFG30", "JFG33", "JFG36"),
    inkl_konf=inkl_konf, op_gruppe=op_gruppe,
    datoFra=datoFra, datoTil=datoTil,
    hastegrad_hybrid=hastegrad_hybrid, malign=malign,
    prikktall = FALSE, pst_kolonne = TRUE)
...
RegData <- RegData |>
  mutate(
    tilgang_tilbakelegging = case_when(
      ncsp_lowercase %in% c("jfg00", "jfg10", "jfg20", "jfg30") ~
        "Lokal tilgang",
      ncsp_lowercase %in%
        c("jfg23", "jfg26", "jfg29", "jfg33", "jfg36") ~ "laparotomi/laparoskopi",
      .default = NA
    ) |> as.factor(),
    tilbakelegging = case_when(
      ncsp_lowercase %in%
        c("jfg00", "jfg20", "jfg23", "jfg26", "jfg29") ~ "Ileostomi",
      ncsp_lowercase %in%
        c("jfg10", "jfg30", "jfg33", "jfg36") ~ "Kolostomi",
      .default = NA
    ) |> as.factor()
  )

outfile <- 'anastomoselekk_ileo_vs_kolo.pdf'
outfile <- paste0(figfolder, outfile)
lekkdata <- norgast::norgastAndelGruppert1Gr(
  valgtVar="Anastomoselekkasje_alle", outfile = outfile,
  Grvar1 = "tilbakelegging",
  tittel = "Andel anastomoselekkasjer",
  RegData = RegData |> filter(FerdigForlop == 1),
  datoFra=paste0(rap_aar, "-01-01"),
  datoTil=paste0(rap_aar, "-12-31"),op_gruppe=14,
  ytxt = "", inkl_konf = TRUE)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgast::norgastAndelGruppert1Gr(
...
outfile <- "anastomoselekk_ileo_vs_kolo_m_undergr.pdf"
outfile <- paste0(figfolder, outfile)
norgast::norgastAndelGruppert2Gr(
  valgtVar="Anastomoselekkasje_alle", outfile = outfile,
  Grvar1 = "tilbakelegging", Grvar2 = "tilgang_tilbakelegging",
  tittel = "Andel anastomoselekkasjer",
  RegData = RegData |> filter(FerdigForlop == 1),
  datoFra=paste0(rap_aar-2, "-01-01"),
  datoTil=paste0(rap_aar, "-12-31"),op_gruppe=14,
...
outfile <- 'AccordionGrad_tilbakelegging.pdf'
outfile <- paste0(figfolder, outfile)
FigAndeler(
  RegData, valgtVar='AccordionGrad', enhetsUtvalg=0, op_gruppe=14,
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}Anastomoselekkasje_tilbakelegging.pdf}
\caption{Anastomoselekkasje\_tilbakelegging.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}Anastomoselekkasje_ileostomi.pdf}
\caption{Anastomoselekkasje\_ileostomi.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}Anastomoselekkasje_kolostomi.pdf}
\caption{Anastomoselekkasje\_kolostomi.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}anastomoselekk_ileo_vs_kolo.pdf}
\caption{anastomoselekk\_ileo\_vs\_kolo.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}anastomoselekk_ileo_vs_kolo_m_undergr.pdf}
\caption{anastomoselekk\_ileo\_vs\_kolo\_m\_undergr.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}AccordionGrad_tilbakelegging.pdf}
\caption{AccordionGrad\_tilbakelegging.pdf}
\end{figure}
