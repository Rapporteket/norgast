...
tunnelplot_UI <- function(id, BrValg){
  ...
  shiny::sidebarLayout(
    ...
    mainPanel(
      ...
      #                     hover = hoverOpts(id = "plot_hover", delay = 100, delayType = "debounce")), height="auto"),
      # uiOutput(ns("hover_info")),
      # downloadButton(ns("lastNedBilde_Tunnelplot"), "Last ned figur"),
      # uiOutput("hover_info_verbatim"))
...
tunnelplot <- function(input, output, session, reshID, RegData, hvd_session){
  ...
  output$fig_tunnel <- renderPlot({
    ...
    ggplot(data=my_data, aes(x=d,y=andel, color=color)) +
      ...
      labs(x = "Antall forløp",  y = "Andel (%)", title ="Tunnelplot") +
