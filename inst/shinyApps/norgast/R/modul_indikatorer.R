  ...
  output$Figur1 <- renderPlot({
    switch(input$valgtVar,
           ...
           "ReLapNarkose" = norgast::norgastIndikator_rapporteket(RegData = RegData[which(RegData$Aar <= as.numeric(input$tilAar_verdi)), ],
                                                                  ...
                                                                  inkl_konf=T, op_gruppe=1, whoEcog = c(0,1),
                                                                  hastegrad_hybrid=1, malign=1, lavDGtekst = "Lav dekningsgrad"),
           "konv_rate" = norgast::norgastIndikator_rapporteket(RegData = RegData[which(RegData$Aar <= as.numeric(input$tilAar_verdi)), ],
                                                               valgtVar = "konv_rate",
                                                               outfile="", tittel="",  width=600, height=700,
                                                               ...
                                                               inkl_konf=T, op_gruppe=1, whoEcog = c(0,1),
                                                               hastegrad_hybrid=1, malign=1, lavDGtekst = "Lav dekningsgrad"),
           "Anastomoselekkasje" = norgast::norgastIndikator_rapporteket(RegData = RegData[which(RegData$Aar <= as.numeric(input$tilAar_verdi)), ],
                                                                        ...
                                                                        inkl_konf=T, op_gruppe=1, whoEcog = c(0,1),
                                                                        hastegrad_hybrid=1, malign=1, lavDGtekst = "Lav dekningsgrad"),
           "mortalitet90" = norgast::norgastIndikator_rapporteket(RegData = RegData[which(RegData$Aar <= as.numeric(input$tilAar_verdi)), ],
