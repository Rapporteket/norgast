...
Tabell <- RegDataAll %>%
  dplyr::filter(Aar == rap_aar) %>%
  dplyr::filter(Tilgang_utvidet %in% 2:5) %>%
  summarise(Robotassistert = sum(Robotassistanse),
            N = n(),
            .by = Sykehusnavn) %>%
  dplyr::arrange(desc(Robotassistert))#
ikkerobot <- data.frame("Sykehusnavn" = "Sykehus uten robotkirurgi",
           "Robotassistert" = 0,
           "N" = sum(Tabell$N[Tabell$Robotassistert == 0]))
Tabell <- Tabell %>%
  dplyr::bind_rows(ikkerobot) %>%
  dplyr::filter(Robotassistert > 0 | Sykehusnavn == "Sykehus uten robotkirurgi") %>%
