...
```{r, anastomoselekkasje per sykehus}



model_samlet <- lme4::glmer(Anastomoselekkasje ~ 1 +  
                              erMann + alder_kat + Robotassistanse +
                              ASA_kat + straaling + (1 | Sykehusnavn) +
                              AvstandAnalVerge_kat + bmi_forenklet + NyStomi,
                            data = RegData %>% filter(!is.na(AvstandAnalVerge_kat),
                                                      !is.na(BMI)), 
                            family = "binomial")
summary(model_samlet)
...
RegData <- RegData_alle |> filter(
  Sykehusnavn %in% c("Ahus", "Haukeland", "OUS-Ullevål", 
                     "SI-Hamar", "SS-Kristiansand",
                     "St.Olavs", "Stavanger", "Skien", 
...
ps.test$post.match.balance
df.coef.robot.mStomi <- ps.test$outcome.model$coefficients %>% exp()
df.confint.robot.mStomi <- confint(ps.test$outcome.model) %>% exp()
graf <- ps.test$graphs
...
tmp <- RegData |> select("erMann", "Alder", "ASA_kat", "straaling",
                         "AvstandAnalVerge_kat", "bmi_forenklet",  
                         "Aar", "Sykehusnavn") |> 
...
matched_data <- MatchIt::matchit(formula = Robotassistanse ~ 1,
                                 ...
                                 caliper = 0.25, std.caliper = T,
                                 # method = "optimal", replace = F,
                                 ratio = 3)
