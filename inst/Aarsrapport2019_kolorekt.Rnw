...
\title{Figurer og tabeller for årsrapport NoRGast 2019 - Kolon og rektum}
\author{NoRGast}
...
NorgastFigAndelStabelGrVar(RegData, valgtVar='AccordionGrad_drenasje', op_gruppe=op_gruppe, hastegrad_hybrid=1, outfile=outfile,
                           Ngrense=10, malign = malign, lavDG = graaUt_kolon)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
...
# outfile <- 'mortalitet90_kolon_aar_refpas2.pdf'
# valgtVar <- 'mortalitet90'
# norgastIndikatorLinn(RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width, height=height,
#                      decreasing=T, terskel=terskel, minstekrav = NA, maal = NA, skriftStr=skriftStr, dagtid = 1, whoEcog = c(0,1),
#                      pktStr=pktStr, legPlass=legPlass, minstekravTxt=minstekravTxt, maalTxt=maalTxt, graaUt=graaUt, lavDG = graaUt_kolon,
#                      inkl_konf=inkl_konf, op_gruppe=op_gruppe, datoFra=datoFra, datoTil=datoTil, hastegrad_hybrid=1, malign=1)
# if (annet_format_ut) {
#   outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{mortalitet90_kolon_aar_refpas.pdf}
\caption{}
\end{figure}
%
% \begin{figure}[ht]
% \centering
% \includegraphics[width=\Sexpr{figstr}\textwidth]{mortalitet90_kolon_aar_refpas2.pdf}
% \caption{}
% \end{figure}
