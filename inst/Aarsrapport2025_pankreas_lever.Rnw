...
dgterskel <- 70
DG <- readr::read_csv2(
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/DGA_Norgast/Sykehus/Lever_sh.csv", locale = locale(encoding = "WINDOWS-1252")) |>
...
DG <- readr::read_csv2(
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/DGA_Norgast/Sykehus/Whipple_sh.csv", locale = locale(encoding = "WINDOWS-1252")) |>
  dplyr::mutate(AvdRESH = substr(AvdRESH, 1, 7),
                AvdRESH = ifelse(AvdRESH == 4204126, 4204084, AvdRESH)) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
...
DG <- readr::read_csv2(
  "C:/Users/kth200/regdata/norgast/aarsrapp2025/DGA_Norgast/Sykehus/Distal_pankreas_sh.csv", locale = locale(encoding = "WINDOWS-1252")) |>
  dplyr::mutate(AvdRESH = substr(AvdRESH, 1, 7),
                AvdRESH = ifelse(AvdRESH == 4204126, 4204084, AvdRESH)) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
