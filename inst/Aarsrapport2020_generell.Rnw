...
\title{Figurer og tabeller for årsrapport NoRGast 2020 - Generell del}
\author{NoRGast}
...
#
# DG_alle <- xlsx::read.xlsx("I:/norgast/vedleggstabeller2019data.xlsx", sheetName = 'Tabell 17', encoding = 'UTF-8', stringsAsFactors=FALSE, startRow = 3)
# DG_alle$Total[DG_alle$Org..nr.==974589095] <- DG_alle$Total[DG_alle$Org..nr.==974588951] +
#   DG_alle$Total[DG_alle$Org..nr.==974589095]
#
# Ant_tot <- RegData[!is.na(RegData$Op_grAarsrapp), ] %>% group_by(AvdRESH) %>% summarise(NoRGast = n()) %>%
#   merge(mapping_npr[, c("orgnr_sh", "AvdRESH", "Sykehusnavn")], by = "AvdRESH", all.x = T)
# DG_alle <- merge(DG_alle[, c("Org..nr.", "Total")], Ant_tot,
#                  by.x = "Org..nr.", by.y = "orgnr_sh")
# DG_alle$dg <- DG_alle$NoRGast/DG_alle$Total*100
# graaUt_alle <- DG_alle$Sykehusnavn[DG_alle$dg <55]
# write.csv2(DG_alle[, c("Sykehusnavn", "NoRGast", "Total", "dg")], "I:/norgast/dg_tot_estimert_2020.csv",
...
write.csv2(Tabell, 'C:/GIT/norgast/inst/tabeller/ant_reg_alle.csv', row.names = T, fileEncoding = 'Latin1')
...
write.csv2(Tabell, 'C:/GIT/norgast/inst/tabeller/ant_reg_oblig.csv', row.names = T, fileEncoding = 'Latin1')
...
write.csv2(Tabell, 'C:/GIT/norgast/inst/tabeller/RegistrenrendeAvd.csv', row.names = T, fileEncoding = 'Latin1')
@
