...
oppsum_stat <- RegData %>% 
  ...
  summarise(
    ...
    "BMI (gjennonsnitt)" = mean(BMI, na.rm = T),
...
pverdier <- pverdier %>% 
  mutate(pverdi = case_match(
    Variabel,
    "Andel kvinner" ~ 
      fisher.test(table(RegData[,c("Anastomoselekkasje", "ASA")]))$p.value,
...
knitr::kable(
  ...
  row.names = FALSE) %>% 
  kableExtra::add_header_above(c(" " = 2, "Anastomoselekkaskje" = 2))
...
aux4 <- summary(model4)$coefficients %>%
  as.data.frame.matrix() 
