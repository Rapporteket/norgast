...
RegData <- NorgastPreprosess(RegData_raa)    # Enkle filtreringer og 
...
RegData <- RegData %>% 
  filter(#HovedDato >= "2020-01-01",
    ...
    WHOECOG != 9,              # Fjerner registreringene med "Ukjent" WHOECOG
    Op_gr == 2,                # Rektumreseksjoner
    Hastegrad == 1,            # Elektive operasjoner
    Hastegrad_tid == 1,        # Ukedag og med anestesistart i normalarbeidstid
...
tab1 <- RegData %>%
  ...
  mutate(Lekkasje_pst = Lekkkasje/N*100,
         MedianAlder = ifelse(Robotassistanse == "-", 
                              median(RegData$Alder), MedianAlder),
         Tum_avst = ifelse(
           Robotassistanse == "-", 
           median(RegData$AvstandAnalVerge, na.rm = T), Tum_avst),
...
```{r}
knitr::kable(tab2, digits = 2, caption = "Andel anastomoselekkasjer for 
robotassistert og konvensjonell laparoskopi") %>% 
...
oppsum_stat <- RegData %>% 
  ...
  summarise(
    ...
    "Andel preoperativ bestråling" = sum(KunStraaleterapi | 
                                           KjemoRadioKombo)/Antall*100,
...
pverdier <- pverdier %>% 
  mutate(pverdi = case_match(
    Variabel,
    "Andel kvinner" ~ 
      prop.test(table(RegData[,c("Anastomoselekkasje", "erMann")]))$p.value,
    "Andel preoperativ bestråling" ~
      prop.test(table(RegData[,c("Anastomoselekkasje", "straaling")]))$p.value,
    "Andel anlagt ny stomi" ~
      prop.test(table(RegData[,c("Anastomoselekkasje", "NyStomi")]))$p.value,
    "Andel robotkirurgi" ~ 
      prop.test(table(RegData[,c("Anastomoselekkasje", 
                                 "Robotassistanse")]))$p.value,
    "Alder (gj.sn)" ~
...
## Ny analyse: Ny anastomose, ja eller nei

```{r}
# Samme datasett som før men med pasienter uten anastomose inkludert
RegData <- NorgastPreprosess(RegData_raa)
RegData <- RegData %>% 
  filter(
    HovedDato >= "2021-01-01",
    HovedDato <= "2023-12-31",
    FerdigForlop_v2 == 1,
    WHOECOG != 9,
    Op_gr == 2,
    Hastegrad == 1,
...
andel_robot <- RegData %>% summarise(ant_ny_anastomose=sum(NyAnastomose==1),
                                     N=n(),
                                     .by=Robotassistanse) %>% 
  janitor::adorn_totals() %>% 
  mutate(pst_ny_anastomose=ant_ny_anastomose/N*100,
         Robotassistanse = case_when(Robotassistanse==1 ~ "Ja", 
                                     Robotassistanse==0 ~ "Nei",
                                     .default = Robotassistanse))

knitr::kable(andel_robot, digits = 1,
             caption = "Andel ny anastomose blant gruppen med og uten 
             robotassistanse. Det er en signifikant større andel 
             robotassistanse i gruppen som ikke får ny anastomose, med 
             p-verdi 0.003 (som vist i påfølgende analyse.")

prop.test(table(RegData[,c("Robotassistanse", "NyAnastomose")]))
