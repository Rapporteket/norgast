...
oppsum_stat <- RegData %>% 
  ...
  summarise(
    Antall = n(),
    "Alder (median)" = median(Alder),
    "Andel kvinnner" = sum(erMann==0)/Antall*100,
    "Andel preoperativ bestråling" = sum(KunStraaleterapi | KjemoRadioKombo)/Antall*100,
    "Tumoravstand fra anus (median)" = median(AvstandAnalVerge, na.rm = T),
    "ASA-score (gjennonsnitt)" = mean(ASA),
  ...
  tr_summarize_output(kolnavn1 = "")
# %>% 
#   mutate(Anastomoselekk_nei = round(Anastomoselekk_nei, 1),
#          Anastomoselekk_ja = round(Anastomoselekk_ja, 1))

knitr::kable(
  oppsum_stat, digits = 1, 
  caption = "Oppsummerende statistikk for de med og uten anastomoselekkasje",
  row.names = FALSE) %>% 
  kableExtra::add_header_above(c(" " = 1, "Anastomoselekkaskje" = 2))
...
model3 <- glm(NyStomi ~ 1 + Robotassistanse + 
