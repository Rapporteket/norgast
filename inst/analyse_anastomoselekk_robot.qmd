...
# formel <- formula(Anastomoselekkasje ~ 1 + Robotassistanse +
#                 NyStomi + erMann + as.factor(WHOECOG) + Alder +
#                   f(Aar, model="ar1") +
#                 as.factor(ASA) + as.factor(Forbehandling) +
#                 AvstandAnalVerge + BMI)
# formel <- formula(Anastomoselekkasje ~ 1 +
#                   f(Alder, model="rw1"))
# 
# 
# model4 <- INLA::inla(formel, data = RegData %>% filter(!is.na(AvstandAnalVerge),
#                                         !is.na(BMI)),
#                      family = "binomial")
# 
# summary(model4)
