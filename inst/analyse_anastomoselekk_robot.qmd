...
frafall <- RegData |> 
  summarise(
    ...
    "WHOECOG > 1" = sum(!(WHOECOG %in% 0:1)),
    "Akuttkirugi" = sum(Hastegrad == 2),          
    "Utenfor normalarbeidstid" = sum(Hastegrad_tid == 0),
    "Åpen kirurgi" = sum(Tilgang == 1),
    "Uten ny anastomose, konvensjonell laparoskopi (inkl. konverterte)" = 
      sum(NyAnastomose != 1 & Robotassistanse == 0 & Tilgang %in% 2:3, na.rm = T),
    "Uten ny anastomose, robotassistert laparoskopi (inkl. konverterte)" = 
      sum(NyAnastomose != 1 & Robotassistanse == 1 & Tilgang %in% 2:3, na.rm = T),
