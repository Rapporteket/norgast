...
@
\captionof{table}{Elektive operasjoner. Reoperasjoner for de ulike operasjonsgruppene inkludert fordeling på årsak til reoperasjon. Gjelder \textbf{hele landet}.}
\end{minipage}
\end{table}
...
tmp <- RegDataAll[RegDataAll$OperasjonsDato >= '2018-06-01',]
tmp$Sykehusnavn <- as.character(tmp$Sykehusnavn)
tmp_kolon <- tmp[tmp$Op_gr == 1 & !is.na(tmp$Hastegrad), ]

# aux <- tmp_kolon %>% group_by(Sykehusnavn) %>% summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0)/n()*100,
...
aux <- tmp_kolon[tmp_kolon$Malign==0, ] %>% group_by(Sykehusnavn) %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            ...
            N=n())
aux2 <- tmp_kolon[tmp_kolon$Malign==0, ] %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
            N=n())
aux <- bind_rows(aux, bind_cols(Sykehusnavn='Totalt', aux2))

print(xtable::xtable(aux, caption='Sykehusvis tabell med grad av overensstemmelse mellom øhjelpsdefinisjoner, benign kolon fra medio 2018 til ut 2019.', digits=c(0,0,1,1,1,1,0)), include.rownames = F)

aux <- tmp_kolon[tmp_kolon$Malign==1, ] %>% group_by(Sykehusnavn) %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
            N=n())
aux2 <- tmp_kolon[tmp_kolon$Malign==1, ] %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
...
print(xtable::xtable(aux, caption='Sykehusvis tabell med grad av overensstemmelse mellom øhjelpsdefinisjoner, malign kolon fra medio 2018 til ut 2019.', digits=c(0,0,1,1,1,1,0)), include.rownames = F)

tmp_rektum <- tmp[tmp$Op_gr == 2 & !is.na(tmp$Hastegrad), ]

aux <- tmp_rektum[tmp_rektum$Malign==0, ] %>% group_by(Sykehusnavn) %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
            N=n())
aux2 <- tmp_rektum[tmp_rektum$Malign==0, ] %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
...
print(xtable::xtable(aux, caption='Sykehusvis tabell med grad av overensstemmelse mellom øhjelpsdefinisjoner, benign rektum fra medio 2018 til ut 2019.', digits=c(0,0,1,1,1,1,0)), include.rownames = F)

aux <- tmp_rektum[tmp_rektum$Malign==1, ] %>% group_by(Sykehusnavn) %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
            N=n())
aux2 <- tmp_rektum[tmp_rektum$Malign==1, ] %>%
  summarise('øhjelp/øhjelp_tid' = sum(Hastegrad==2 & Hastegrad_tid==0),
            'elektiv/elektiv_tid' = sum(Hastegrad==1 & Hastegrad_tid==1),
            'øhjelp/elektiv_tid' = sum(Hastegrad==2 & Hastegrad_tid==1),
            'elektiv/øhjelp_tid' = sum(Hastegrad==1 & Hastegrad_tid==0),
...
print(xtable::xtable(aux, caption='Sykehusvis tabell med grad av overensstemmelse mellom øhjelpsdefinisjoner, malign rektum fra medio 2018 til ut 2019.', digits=c(0,0,1,1,1,1,0)), include.rownames = F)
@
