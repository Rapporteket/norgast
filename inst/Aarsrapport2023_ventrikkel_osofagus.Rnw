...
\title{Figurer og tabeller for årsrapport NORGAST 2023 - Ventrikkel og øsofagus}
\author{NORGAST}
...
# graaUt_ventrikkel <- DG$Sykehusnavn[DG$DG < 60 & DG$Op_gr == 4]
# graaUt_osofagus <- DG$Sykehusnavn[DG$DG < 60 & DG$Op_gr == 3]

sh_norgast <- RegDataAll[match(unique(RegDataAll$AvdRESH), RegDataAll$AvdRESH), c("AvdRESH", "Sykehusnavn")]

DG <- read.csv2('~/mydata/norgast/DG_Sykehus/Ventrikkel_sh.csv') %>%
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) %>%
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_ventrikkel <- DG$Sykehusnavn[DG$DG_norgast < 70 | (DG$Begge + DG$Kun_norgast) <= 10 ]
...
#     Op_gr %in% c(3,4) &
#       (n_npr >= 10 | AvdRESH %in%
#          c(107440, 106168, 700922, 601225, 108162))) %>%
...
annet_format_ut <- T
ut_format <- 'svg'
figfolder <- "~/mydata/norgast/fig_aarsrapp2023/ventrikkel_oefsofagus/"
...
#   lavDG = graaUt_osofagus, inkl_konf=inkl_konf, op_gruppe=op_gruppe, datoFra=datoFra,
...
#
# outfile <- paste0(figfolder, 'Tilgang_ventrikkel_aar_robot.pdf')
# valgtVar <- 'LapTilgang2'
...
#   lavDG = graaUt_ventrikkel, lavDGtekst = dg_tekst, inkl_konf=inkl_konf,
#   op_gruppe=op_gruppe, datoFra=datoFra, datoTil=datoTil,
#   hastegrad_hybrid=hastegrad_hybrid, malign=malign, Grvar2 = "Robot")
# if (annet_format_ut) {
#   outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
#   norgastIndikator_gruppert(
#     RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
...
norgastIndikator_rapporteket(
  RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
  height=height, decreasing=T, terskel=terskel, minstekrav = minstekrav, maal = 15,
  skriftStr=skriftStr, lavDG = graaUt_ventrikkel, lavDGtekst = dg_tekst,
  pktStr=pktStr, legPlass=legPlass, minstekravTxt=minstekravTxt, maalTxt=maalTxt,
  graaUt=graaUt, maalretn = 'lav', inkl_konf=inkl_konf, op_gruppe=op_gruppe,
  datoFra=datoFra, datoTil=datoTil, hastegrad_hybrid=1, malign=malign)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
    RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
...
# RegDataAll$maligndiag <- factor(RegDataAll$Malign, levels = 0:1, labels = c("Benign", "Malign"))
# outfile <- paste0(figfolder, 'ReLapNarkose_ventrikkel_aar_gruppertmalign.pdf')
# valgtVar <- 'ReLapNarkose'
# norgastIndikator_gruppert(
#   RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
#   height=height, decreasing=T, terskel=terskel, minstekrav = minstekrav, maal = 15,
...
#   graaUt=graaUt, maalretn = 'lav', inkl_konf=inkl_konf, op_gruppe=op_gruppe,
#   datoFra=datoFra, datoTil=datoTil, hastegrad_hybrid=1, malign=malign,
#   Grvar2 = "maligndiag", prikktall = FALSE, inkl_N = FALSE)
# if (annet_format_ut) {
#   outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
#   norgastIndikator_gruppert(
#     RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
...
outfile <- "ReLapNarkose_ventrikkel_gruppertmalign_nasjonal.pdf"
outfile <- paste0(figfolder, outfile)
norgastIndikator_nasjonal(
  RegDataAll, valgtVar = "ReLapNarkose", outfile=outfile, tittel=tittel, width=width,
  height=height, decreasing=decreasing, terskel=terskel, minstekrav = minstekrav,
  maal = maal, skriftStr=skriftStr, pktStr=pktStr, legPlass=legPlass,
  minstekravTxt=minstekravTxt, maalTxt=maalTxt, graaUt=graaUt,
  lavDG = graaUt_ventrikkel, lavDGtekst = dg_tekst, inkl_konf=inkl_konf,
  op_gruppe=op_gruppe, datoFra=datoFra, datoTil=datoTil,
  hastegrad_hybrid=hastegrad_hybrid, malign=malign, Grvar = "maligndiag")
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_nasjonal(
    RegDataAll, valgtVar = "ReLapNarkose", outfile=outfile, tittel=tittel, width=width,
...
# outfile <- paste0(figfolder, 'mortalitet90_ventrikkel_aar_gruppertrobot.pdf')
# valgtVar <- 'mortalitet90'
# norgastIndikator_gruppert(
#   RegDataAll, valgtVar = valgtVar, outfile=outfile, tittel=tittel, width=width,
#   height=height, decreasing=T, terskel=terskel, minstekrav = 8, maal = 5,
#   skriftStr=skriftStr, lavDG = graaUt_ventrikkel, lavDGtekst = dg_tekst,
#   pktStr=pktStr, legPlass="topleft", minstekravTxt=minstekravTxt, maalTxt=maalTxt,
#   graaUt=graaUt, inkl_konf=inkl_konf, op_gruppe=op_gruppe, datoFra=datoFra,
#   datoTil=datoTil, hastegrad_hybrid=1, malign=malign,
#   kun_ferdigstilte = F, Grvar2 = "Robot", prikktall = FALSE, inkl_N = F,
#   rotermaaltxt=45)
# if (annet_format_ut) {
#   outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
#   norgastIndikator_gruppert(
...
#     graaUt=graaUt, inkl_konf=inkl_konf, op_gruppe=op_gruppe, datoFra=datoFra,
#     datoTil=datoTil, hastegrad_hybrid=1, malign=malign,
#     kun_ferdigstilte = F, Grvar2 = "Robot", prikktall = FALSE,
#     inkl_N = F, rotermaaltxt=45)
# }
...
norgastIndikator_rapporteket(
  ...
  minstekravTxt=minstekravTxt, maalTxt=maalTxt, graaUt=graaUt, maalretn='lav',
  inkl_konf=inkl_konf, op_gruppe=op_gruppe, datoFra='2018-05-01', datoTil=datoTil,
  hastegrad_hybrid=hastegrad_hybrid, malign=1, hastegrad = 1)
if (annet_format_ut) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-3), ut_format)
  norgastIndikator_rapporteket(
  RegDataAll, valgtVar = valgtVar, outfile=outfile, width=width, height=height,
...
% \begin{figure}[ht]
% \centering
% \includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}ReLapNarkose_ventrikkel_aar_gruppertmalign.pdf}
% \caption{ReLapNarkose\_ventrikkel\_aar\_gruppertmalign.pdf}
% \end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}ReLapNarkose_ventrikkel_gruppertmalign_nasjonal.pdf}
\caption{ReLapNarkose\_ventrikkel\_gruppertmalign\_nasjonal.pdf}
\end{figure}
