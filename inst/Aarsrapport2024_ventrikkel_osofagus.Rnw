...
\title{Figurer og tabeller for årsrapport NORGAST 2024 - Ventrikkel og øsofagus}
\author{NORGAST}
...
DG <- read.csv2(system.file("extdata/dg2024.csv", package = "norgast"),
                fileEncoding = "UTF-8") |>
  dplyr::filter(Op_gr == 4) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_ventrikkel <- DG$Sykehusnavn[DG$DG < dgterskel]

DG2 <- read.csv2(system.file("extdata/dg2024.csv", package = "norgast"),
                fileEncoding = "UTF-8") |>
  dplyr::filter(Op_gr == 3) |>
  merge(sh_norgast, by = "AvdRESH", all.x = TRUE) |>
  dplyr::filter(!is.na(Sykehusnavn))
graaUt_osofagus <- DG2$Sykehusnavn[DG2$DG < dgterskel]
...
annet_format_ut <- T
ut_format <- 'svg'
figfolder <- "C:/Users/kth200/OneDrive - Helse Nord RHF/Dokumenter/regdata/norgast/aarsrapp2024/ventrikkel_oefsofagus/"
