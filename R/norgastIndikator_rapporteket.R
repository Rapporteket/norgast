...
norgastIndikator_rapporteket <-
  function(
    ...
    KI <- binomkonf(
      AntTilfeller[rekkefolge, dim(andeler)[2]],
      N[, dim(andeler)[2]]
    ) * 100
    KI[, is.na(andeler[, dim(andeler)[2]])] <- NA
    pst_txt <- paste0(sprintf(
      paste0("%.", desimaler_pst, "f"),
    ...
    FigTypUt <- rapFigurer::figtype(
      outfile = outfile, width = width, height = height,
    ...
    if (dim(andeler)[2] == 2) {
      ...
      if (legPlass == "nede") {
        legend(
          x = 82, y = ypos[2] + 1, xjust = 0, cex = cexgr, bty = "n",
    ...
    if (prikktall) {
      ...
      text(
        x = andeler[, 1], y = ypos, labels = pst_txt_prikk,
...
norgastIndikator_gruppert <-
  function(RegData, valgtVar, tittel = "",
    ...
    if ("Variabel" %in% names(RegData)) {
      ...
      Samlet <- RegData %>%
        dplyr::summarise(
          antall = sum(!!sym(valgtVar), na.rm = T),
    ...
    aux <- Samlet %>%
      dplyr::select(-c(N, andel)) %>%
      tidyr::pivot_wider(
        id_expand = TRUE, names_from = Aar, values_from = antall,
    ...
    aux <- Samlet %>%
      dplyr::select(-c(antall, andel)) %>%
      tidyr::pivot_wider(
        id_expand = TRUE, names_from = Aar, values_from = N,
        values_fill = 0, names_sort = TRUE
      ) %>%
      dplyr::mutate(tot = rowSums(across(-c(1, 2))))
    names(aux)[dim(aux)[2]] <- paste0(names(aux)[3], "-", names(aux)[dim(aux)[2] - 1])
    ...
    plotdata <- andeler %>%
      select(1, 2, 4) %>%
      pivot_wider(
        id_expand = TRUE, names_from = !!sym(Grvar2), values_from = 3,
        values_fill = 0, names_sort = TRUE
      ) # %>% dplyr::arrange(across(ncol(.)))

    tmp <- plotdata[, 2:dim(plotdata)[2]] %>%
      as.matrix() %>%
      t()
    ...
    if (inkl_N) {
      ...
      pst_txt <- paste0(sprintf("%.0f", andeler[[4]]), " %")
    }
    # pst_txt <- paste0(sprintf('%.0f', andeler[[4]]), ' %', ', N = ', N[[4]])
    ...
    plotdata <- cbind(
      matrix(rep(NA, nlevels(andeler[[2]])),
    ...
    KInew <- rbind(
      matrix(c(NA, NA), nrow = nlevels(andeler[[2]]) * lbunn, ncol = 2), KInew,
      matrix(c(NA, NA), nrow = nlevels(andeler[[2]]) * ltop, ncol = 2)
    )

    ypos <- barplot(plotdata,
      beside = T, las = 1,
      xlim = c(0, xmax),
    ...
    if (inkl_konf) {
      arrows(
        x0 = KInew[, 1], y0 = ypos, x1 = KInew[, 2], y1 = ypos,
        length = 0.5 / max(ypos), code = 3, angle = 90, lwd = 1.8, col = "gray"
      )
      legend("bottom",
        cex = 0.9 * cexgr, bty = "n",
    ...
    mtext(paste0("N, ", names(N)[4]),
      side = 2, line = 0.4, las = 1,
    ...
    points(
      y = ypos, x = c(
        rep(NA, lbunn * ngrvar2level), andeler[[3]],
    ...
    legend(legPlass,
      inset = c(inset, 0), cex = 0.7 * cexgr, bty = "n",
    ...
    aux2 <- data.frame(
      ypos = (ypos + c(tail(ypos, -1), head(ypos, 1))) / 2,
      tekst = c(
        rep(NA, lbunn * ngrvar2level),
        pst_txt, rep(NA, ltop * ngrvar2level)
      )
...
norgastIndikator_nasjonal <-
  function(RegData, valgtVar, tittel = "", width = 800, height = 700,
    ...
    if ("Variabel" %in% names(RegData)) {
      Tabell <- RegData %>% dplyr::summarise(
        antall = sum(Variabel),
        N = dplyr::n(),
        .by = c(Grvar, Aar)
      )
    ...
    if (dim(andeler)[2] == 2) {
      ...
      if (legPlass == "nede") {
        legend("bottomright",
          cex = 0.9 * cexgr, bty = "n", # bg='white', box.col='white',
          lwd = c(NA, NA), pch = c(19, 15), pt.cex = c(1.2, 1.8), col = c("black", farger[3]),
          legend = names(N), ncol = 1
        )
      }
