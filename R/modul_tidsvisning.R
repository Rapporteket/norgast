...
#'
#' @export
tidsvisning_server <- function(id, reshID, RegData, userRole, hvd_session, BrValg) {
  moduleServer(
    id,
      ...
      output$Tabell_tid <- function() {
        ...
        if (input$enhetsUtvalg == 1) {
          Tabell_tid <- dplyr::tibble(
            Tidsperiode = utdata$Tidtxt, Antall = round(utdata$Andeler$AndelHoved * utdata$NTid$NTidHoved / 100),
