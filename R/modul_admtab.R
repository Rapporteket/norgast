  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      antskjema <- function() {
        ...
        tmp <- merge(
          skjemaoversikt[skjemaoversikt$Skjemanavn=='Registrering',
      ...
      andre_adm_tab <- function() {
        if (input$adm_tidsenhet == 1) {
          ...
          aux$mnd <- factor(format(aux$HovedDato, format='%b-%y'),
                            levels = format(seq(fraDato, tilDato, by="month"),
                                            "%b-%y"))
          ...
          ant_skjema <- switch (
            ...
            '1' = as.data.frame.matrix(
              addmargins(
                table(aux[which(aux$SkjemaStatus==1 &
            ...
            '4' = as.data.frame.matrix(
              addmargins(
                table(aux[which(aux$SkjemaStatus==0) ,
                          c('Sykehusnavn', 'mnd')])))
        ...
        if (rapbase::isRapContext()) {
          ...
          if (req(input$admtabeller) == "id_ant_tid") {
            mld_adm1 <- paste0(
              ...
              c('Ferdige forløp', 'Oppfølging i kladd',
