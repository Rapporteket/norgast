    ...
    sidebarPanel(
      ...
      shinyjs::hidden(
        selectInput(
          ...
          label = "Skjemastatus",
          choices = c(
            "Ferdige forløp" = 1, "Oppfølging i kladd" = 2,
...
#' Serverdel av modul for Administrative tabeller-fane i NORGAST sin shiny-app
#' på Rapporteket
#'
#' Kun til bruk i Shiny
#'
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      antskjema <- function() {
        ...
        tmp <- merge(
          skjemaoversikt[
            skjemaoversikt$Skjemanavn == "Registrering",
            c(
              "ForlopsID", "SkjemaStatus", "HovedDato",
              "OpprettetDato", "Sykehusnavn", "AvdRESH",
              "Op_gr", "Hovedoperasjon"
            )
          ],
          skjemaoversikt[
            skjemaoversikt$Skjemanavn == "Oppfolging/Innleggelse",
        ...
        tmp$HovedDato[is.na(tmp$HovedDato)] <-
          tmp$OpprettetDato[is.na(tmp$HovedDato)]
        if (!is.null(input$op_gruppe)) {
          tmp <- tmp[which(tmp$Op_gr %in% as.numeric(input$op_gruppe)), ]
        }
        if (!is.null(input$ncsp_verdi)) {
          tmp <- tmp[which(substr(tmp$Hovedoperasjon, 1, 5) %in%
            input$ncsp_verdi), ]
        }
        if (!is.null(input$valgtShus) & userRole() == "SC") {
          tmp <- tmp[tmp$AvdRESH %in% as.numeric(input$valgtShus), ]
        }
        ...
        aux <- tmp %>%
          ...
          dplyr::summarise(
            "Ferdige forløp" = sum(
              SkjemaStatus == 1 &
                (SkjemaStatus_oppf == 1 | is.na(SkjemaStatus_oppf)),
              na.rm = T
            ),
        ...
        aux2 <- tmp %>%
          ...
          dplyr::summarise(
            "Ferdige forløp" = sum(
              SkjemaStatus == 1 & (SkjemaStatus_oppf == 1 |
                is.na(SkjemaStatus_oppf)),
              na.rm = T
            ),
            "Oppfølging i kladd" = sum(SkjemaStatus == 1 &
              SkjemaStatus_oppf == 0, na.rm = T),
        ...
        ant_skjema <- dplyr::bind_rows(
          aux, dplyr::bind_cols(dplyr::tibble(Sykehusnavn = "Totalt"), aux2)
        )
      ...
      andre_adm_tab <- function() {
        if (input$adm_tidsenhet == 1) {
          ...
          tmp$SkjemaStatus_oppf[tmp$SkjemaStatus_oppf == -1] <- 0
          tmp$HovedDato[is.na(tmp$HovedDato)] <-
            as.Date(tmp$OpprettetDato[is.na(tmp$HovedDato)])
          if (!is.null(input$op_gruppe)) {
            tmp <- tmp[which(tmp$Op_gr %in% as.numeric(input$op_gruppe)), ]
          }
          if (!is.null(input$ncsp_verdi)) {
          ...
          aux <- tmp
          aux$mnd <- factor(format(aux$HovedDato, format = "%b-%y"),
            levels = format(
              seq(fraDato, tilDato, by = "month"),
              "%b-%y"
            )
          )
          ...
          ant_skjema <- switch(req(input$regstatus_tid),
            "1" = as.data.frame.matrix(
              addmargins(
                table(aux[
                  which(aux$SkjemaStatus == 1 &
                    (aux$SkjemaStatus_oppf == 1 |
