...
saml_andeler_ui <- function(id) {
  ...
  fluidRow(
    ...
    column(
      ...
      fluidRow(
        ...
        column(3,
          offset = 1,
          selectInput(
            inputId = ns("tidsenhet"), label = "Velg tidsenhet",
    ...
    column(
      ...
      sliderInput(
        inputId = ns("PRS2"), label = "mE-PASS", min = 0, max = 2.2,
        value = c(0, 2.2), step = 0.05
      ),
      uiOutput(outputId = ns("ASA2_ui")),
      selectInput(
        inputId = ns("modGlasgow2"),
        label = "Modified Glasgow score",
        choices = 0:2, multiple = TRUE
      ),
    ...
    function(input, output, session) {
        ...
        if (!is.null(input$op_gruppe)) {
          selectInput(
            ...
            choices = if (!is.null(input$op_gruppe)) {
              setNames(
                substr(sort(unique(RegData$Hovedoperasjon[RegData$Op_gr %in% as.numeric(input$op_gruppe)])), 1, 5),
      ...
      output$ncsp2 <- renderUI({
        ...
        if (!is.null(input$op_gruppe2)) {
          selectInput(
            inputId = ns("ncsp_verdi2"), label = "NCSP koder (velg en eller flere)",
            choices = if (!is.null(input$op_gruppe2)) {
              setNames(
                substr(sort(unique(RegData$Hovedoperasjon[RegData$Op_gr %in% as.numeric(input$op_gruppe2)])), 1, 5),
      ...
      output$Figur_andeler <- renderPlot(
        {
          input$goButton
          isolate(norgast::NorgastFigAndelTid_SammenligUtvalg(
      ...
      output$Tabell_utvalg <- function() {
        input$goButton
        utdata <- isolate(norgast::NorgastFigAndelTid_SammenligUtvalg(
          plotdata = utvalgsfunksjon(), tidsenhet = input$tidsenhet,
          inkl_konf = input$inkl_konf,
          datoFra = input$datovalg[1], datoTil = input$datovalg[2],
          fra0 = input$fra0, inkl_tall = input$inkl_tall
        ...
        if (is.na(utdata$Konf$Utvalg2[1])) {
          Tabell_tid <- dplyr::tibble(
            Tidsperiode = utdata$andeler$TidsEnhet,
          ...
          Tabell_tid <- dplyr::tibble(
            Tidsperiode = utdata$andeler$TidsEnhet, Antall = utdata$antall[["1"]],
          ...
          names(Tabell_tid) <- c(
            "Tidsperiode", "Antall", "N", "Andel (%)", "KI_nedre", "KI_ovre", "Antall", "N", "Andel (%)",
            "KI_nedre", "KI_ovre"
          )
          Tabell_tid %>%
