...
NorgastFigAndelStabelGrVar <- function(
  ...
  if (valgtVar == "AccordionGrad_drenasje") {
    ...
    RegData$AccordionGrad_drenasje[
      which(RegData$AccordionGrad_drenasje == 3 & RegData$KunDrenasje == 1)
    ] <- 2
  }
  ...
  if (max(Ngr) < Ngrense) { # Dvs. hvis ALLE er mindre enn grensa.
    ...
    FigTypUt <- rapFigurer::figtype(
      outfile,
      height = 3 * 800,
      fargepalett = NorgastUtvalg$fargepalett
    ) # res=96,
    farger <- FigTypUt$farger
    if (krgfarger) {
      farger <- rev(c(
        "#4A7695", "#67B6C8", "#EBBE33",
        "#F29D71", "#EDEDED"
      ))
    }
    if (snufarger) {
    ...
    if (length(legendTxt) == 5) {
      farger <- c("pink", farger)
    }
    ...
    pos <- barplot(t(AndelerGr),
      horiz = T, beside = FALSE,
    ...
    mtext(
      at = pos, latex2exp::TeX(grtxt_bold),
      side = 2, las = 1, cex = 1 * skriftStr, adj = 1, line = 0.25
    )
    text(
      x = 0.005 * xmax, y = pos, Ngrtxt, las = 1,
      cex = 0.8 * skriftStr, adj = 0, lwd = 3
    ) # , col=farger[4]	c(Ngrtxt[sortInd],''),
    x_pos_landet <- cumsum(
    ...
    title(tittel,
      line = 1.5, font.main = 1,
      cex.main = 1.4 * skriftStr, xpd = TRUE
    )

    # Tekst som angir hvilket utvalg som er gjort
    mtext(utvalgTxt,
      side = 3, las = 1, cex = 0.7 * skriftStr, adj = 0,
