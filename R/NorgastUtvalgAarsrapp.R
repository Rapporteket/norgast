...
NorgastUtvalgAarsrapp <- function(RegData, datoFra, datoTil, minald, maxald, erMann, elektiv, BMI,
                             valgtShus='', tilgang=99, minPRS=0, maxPRS=2, ASA='', whoEcog='',
                             forbehandling=99, malign=99, fargepalett='BlaaRapp', OpGr='', ncsp='')
{
...
#                          which(RegData$Variabel != '') %i% which(!is.na(RegData$Variabel)) %i% which(!is.nan(RegData$Variabel))
  ...
  indOp_gr <- if (OpGr[1] != '') {which(RegData$Op_grAarsrapp %in% OpGr)} else {indOp_gr <- 1:Ninn}
  ...
  utvalgTxt <- c(paste('Operasjonsdato: ',
                 ...
                 if (OpGr[1] != '') {paste0('Operasjonsgruppe(r): ', paste(OpGr, collapse=','))},
