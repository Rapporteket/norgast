...
traktplot_ui <- function(id) {
  ...
  shiny::sidebarLayout(
    sidebarPanel(
      ...
      div(
        id = ns("br_kontroller1"),
        checkboxInput(
          inputId = ns("referansepasient"),
    ...
    function(input, output, session) {
      ...
      output$settMaalnivaa <- renderUI({
        ...
        if (input$aktiver_justering) {
          numericInput(
            inputId = ns("settMaalnivaa_verdi"), "Midtverdi", min = 0, max = 100,
            value = round(vals$benchmark, 2), step = 0.01
          ) # , ticks = F
        }
      })
      ...
      observe(
        if (req(input$referansepasient)) {
          shiny::updateSelectInput(
            inputId = "hastegrad_hybrid", label = "Hastegrad (hybrid)",
            choices = c("Ikke valgt" = 99, "Elektiv" = 1, "Akutt" = 0), selected = 1
          )
          shiny::updateSelectInput(
            inputId = "malign", label = "Diagnose",
            choices = c("Ikke valgt" = 99, "Malign" = 1, "Benign" = 0), selected = 1
          )
          shiny::updateSelectInput(
            inputId = "whoEcog", label = "WHO ECOG score",
      ...
      observeEvent(input$plot_click, {
        ...
        if (nrow(point) != 0) {
          vals$shus$color <- ifelse(vals$shus$Sykehusnavn == point$Sykehusnavn, "red", "blue")
        }
      })
        ...
        if (!is.null(traktdata()$my_data)) {
          ...
          ggplot2::ggplot(data = my_data, ggplot2::aes(x = Sykehusnavn, y = andel, fill = color)) +
            ...
            ggplot2::scale_fill_manual(
              values = c("red" = "red", "blue" = farger[1]),
              na.value = "red"
            )
        }
      })
      ...
      output$click_info_verbatim <- renderUI({
        if (dim(vals$shus)[1] > 0) {
          ...
          wellPanel(
            HTML(paste0(
              "<b> Avdeling: </b>", my_data$Sykehusnavn[my_data$color == "red" | is.na(my_data$color)], "<br/>",
