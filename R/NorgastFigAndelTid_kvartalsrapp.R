...
#' @export
#'
NorgastFigAndelTid_kvartalsrapp <- function(RegData = 0, valgtVar = "ReLapNarkose", datoFra = "2014-01-01", datoTil = "2050-12-31",
  ...
  } # {indHovedUt <- which(RegData$AvdRESH != reshID)}
  ...
  if (!(inkl_konf %in% c(0, 1))) {
    inkl_konf <- PlotParams$inkl_konf
  }
  ...
  if (length(indHoved) < 10 | (medSml == 1 & length(indRest) < 10)) {
    ...
    if (inkl_konf == 1) {
      ...
      if (medSml == 1) {
        ...
        legend("top",
          cex = 0.9, bty = "o", bg = "white", box.col = "white", lty = c(NA, 2, 2),
          lwd = c(NA, 2, 2), pch = c(15, NA, NA), pt.cex = c(2, 1, 1), col = c(fargeRestRes, farger[1], farger[3]),
          legend = c(
            paste0("95% konfidensintervall for ", smltxt, ", N=", sum(NTidRest, na.rm = T)),
            paste0(
              "Gjennomsnitt hele perioden, ", shtxt, " = ", sprintf("%.1f", AndelHovedGjsn), " (",
              sprintf("%.1f", binomkonf(sum(NTidHendHoved), sum(NTidHoved))[1] * 100), "-",
              sprintf("%.1f", binomkonf(sum(NTidHendHoved), sum(NTidHoved))[2] * 100), ")"
            ),
            paste0(
              "Gjennomsnitt hele perioden, ", smltxt, " = ", sprintf("%.1f", AndelRestGjsn), " (",
      ...
      mtext(sprintf("%.1f", AndelHovedGjsn), side = 4, at = AndelHovedGjsn, las = 1, cex = 0.9, adj = 0, col = fargeHoved, line = 0)
      ...
      if (medSml == 1) {
        ...
        legend("topleft",
          border = NA, c(
            shtxt, paste0(smltxt, ", gj.snitt", " (N=", NSmlRes, ")"),
            paste0(shtxt, ", gj.snitt", " (N=", NHovedRes, ")")
          ),
          bty = "n", lty = c(1, 1, 2), ncol = 2, cex = cexleg,
          col = c(fargeHoved, fargeRest, fargeHoved), lwd = c(3, 3, 2)
        )
        lines(range(xskala), rep(AndelRestGjsn, 2), col = fargeRest, lwd = 2, lty = 1)
        mtext(sprintf("%.1f", AndelRestGjsn), side = 4, at = AndelRestGjsn, las = 1, cex = 0.9, adj = 0, col = fargeRest, line = 0)
      } else {
