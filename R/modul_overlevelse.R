...
overlevelse_ui <- function(id) {
  ...
  fluidRow(
    column(
      ...
      selectInput(
        inputId = ns("elektiv"), label = "Tidspunkt for operasjonsstart",
        choices = c(
          "Ikke valgt" = 99, "Innenfor normalarbeidstid" = 1,
          "Utenfor normalarbeidstid" = 0
        )
      ),
      selectInput(
        inputId = ns("hastegrad"), label = "Hastegrad",
        choices = c("Ikke valgt" = 99, "Elektiv" = 1, "Akutt" = 2)
      ),
      selectInput(
        inputId = ns("hastegrad_hybrid"),
      ...
      uiOutput(ns("BMI_ui")),
      sliderInput(
        inputId = ns("PRS"), label = "mE-PASS", min = 0, max = 2.2,
        value = c(0, 2.2), step = 0.05
      ),
      uiOutput(ns("ASA_ui")),
      selectInput(
        inputId = ns("modGlasgow"), label = "Modified Glasgow score",
        choices = 0:2, multiple = TRUE
      ),
      uiOutput(ns("whoEcog_ui")),
      selectInput(
        inputId = ns("forbehandling"),
        label = "Onkologisk forbehandling", multiple = TRUE,
        choices = c(
          "Cytostatika" = 1, "Stråleterapi" = 2,
          "Komb. kjemo/radioterapi" = 3, "Ingen" = 4
        )
      ),
      selectInput(
        inputId = ns("malign"), label = "Diagnose",
        choices = c("Ikke valgt" = 99, "Malign" = 1, "Benign" = 0)
      ),
      uiOutput(outputId = ns("icd")),
      selectInput(
        inputId = ns("accordion"), label = "Accordiongrad",
        multiple = TRUE,
    ...
    column(
      ...
      sliderInput(
        inputId = ns("alder2"), label = "Alder", min = 0,
        max = 120, value = c(0, 120)
      ),
      selectInput(
        inputId = ns("erMann2"), label = "Kjønn",
        choices = c("Begge" = 99, "Kvinne" = 0, "Mann" = 1)
      ),
      selectInput(
        inputId = ns("elektiv2"),
      ...
      selectInput(
        inputId = ns("hastegrad2"), label = "Hastegrad",
        choices = c("Ikke valgt" = 99, "Elektiv" = 1, "Akutt" = 2)
      ),
      selectInput(
        inputId = ns("hastegrad_hybrid2"),
        label = "Hastegrad, hybrid (bruker hastegrad når den finnes,
                  ellers tidspkt for op.start)",
        choices = c("Ikke valgt" = 99, "Elektiv" = 1, "Akutt" = 0)
      ),
      ...
      uiOutput(ns("BMI2_ui")),
      sliderInput(
        inputId = ns("PRS2"), label = "mE-PASS", min = 0,
        max = 2.2, value = c(0, 2.2), step = 0.05
      ),
      uiOutput(ns("ASA2_ui")),
      selectInput(
        inputId = ns("modGlasgow2"), label = "Modified Glasgow score",
        choices = 0:2, multiple = TRUE
      ),
      uiOutput(ns("whoEcog2_ui")),
      selectInput(
        inputId = ns("forbehandling2"),
        label = "Onkologisk forbehandling", multiple = TRUE,
        choices = c(
          "Cytostatika" = 1, "Stråleterapi" = 2,
          "Komb. kjemo/radioterapi" = 3, "Ingen" = 4
      ...
      selectInput(
        inputId = ns("malign2"), label = "Diagnose",
        choices = c("Ikke valgt" = 99, "Malign" = 1, "Benign" = 0)
      ),
      uiOutput(outputId = ns("icd2")),
      selectInput(
        inputId = ns("accordion2"), label = "Accordiongrad",
        multiple = TRUE,
        choices = c("<3" = 1, "3" = 3, "4" = 4, "5" = 5, "6" = 6)
      ),
      tags$hr(),
...
#'
#' @export
overlevelse_server <- function(id, reshID, RegData,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$valgtShus2_ui <- renderUI({
        ...
        if (userRole() == "SC") {
          selectInput(
            inputId = ns("valgtShus2"), label = "Velg sykehus",
            choices = BrValg$sykehus, multiple = TRUE
          )
        }
      })
      ...
      output$op_gruppe_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("op_gruppe"), label = "Velg reseksjonsgruppe(r)",
      ...
      output$op_gruppe2_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("op_gruppe2"), label = "Velg reseksjonsgruppe(r)",
          choices = BrValg$reseksjonsgrupper, multiple = TRUE
        )
      })
      ...
      output$ASA2_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("ASA2"), label = "ASA-grad",
          choices = BrValg$ASA_valg, multiple = TRUE
        )
      })
      ...
      output$whoEcog_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("whoEcog"), label = "WHO ECOG score",
      ...
      output$whoEcog2_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("whoEcog2"), label = "WHO ECOG score",
          choices = BrValg$whoEcog_valg, multiple = TRUE
        )
      })
      ...
      output$icd <- renderUI({
        ...
        Utvalg1 <- NorgastUtvalg(
          RegData = RegData,
          op_gruppe = if (!is.null(input$op_gruppe)) {
            input$op_gruppe
          } else {
            ""
          },
          ncsp = if (!is.null(input$ncsp_verdi)) {
            input$ncsp_verdi
          } else {
            ""
          },
        ...
        if (!is.null(diagnoser)) {
          selectInput(
            inputId = ns("icd_verdi"), label = "Spesifiser ICD-10 koder (velg en eller flere)",
      ...
      output$icd2 <- renderUI({
        ...
        Utvalg1 <- NorgastUtvalg(
          RegData = RegData,
          op_gruppe = if (!is.null(input$op_gruppe2)) {
            input$op_gruppe2
          } else {
            ""
          },
          ncsp = if (!is.null(input$ncsp_verdi2)) {
            input$ncsp_verdi2
          } else {
            ""
          },
        ...
        if (!is.null(diagnoser)) {
          selectInput(
            inputId = ns("icd_verdi2"), label = "Spesifiser ICD-10 koder (velg en eller flere)",
            choices = diagnoser, multiple = TRUE
          )
        }
      })
      ...
      calc_overlevelse <- function() {
        ...
        Utvalg1 <- shiny::isolate(
          NorgastUtvalg(
            ...
            tilgang_utvidet = if (!is.null(input$tilgang_utvidet)) {
              input$tilgang_utvidet
            } else {
              ""
            },
            minPRS = as.numeric(input$PRS[1]),
            maxPRS = as.numeric(input$PRS[2]),
            ASA = if (!is.null(input$ASA)) {
              input$ASA
            } else {
              ""
            },
            modGlasgow = if (!is.null(input$modGlasgow)) {
              input$modGlasgow
            } else {
              ""
            },
            whoEcog = if (!is.null(input$whoEcog)) {
              input$whoEcog
            } else {
              ""
            },
            ...
            icd = if (!is.null(input$icd_verdi)) {
              input$icd_verdi
            } else {
              ""
            },
        ...
        shiny::isolate(if (!is.null(input$enhetsUtvalg)) {
          if (input$enhetsUtvalg == 2) {
            Utvalg1data <- Utvalg1data[which(Utvalg1data$AvdRESH == reshID()), ]
          }
        })
        ...
        Utvalg2 <- shiny::isolate(
          NorgastUtvalg(
            ...
            maxald = as.numeric(input$alder2[2]),
            erMann = as.numeric(input$erMann2),
            elektiv = as.numeric(input$elektiv2),
            hastegrad = as.numeric(input$hastegrad2),
            hastegrad_hybrid = as.numeric(input$hastegrad_hybrid2),
            BMI = if (!is.null(input$BMI2)) {
              input$BMI2
            } else {
              ""
            },
            valgtShus = if (!is.null(input$valgtShus2)) {
              input$valgtShus2
            } else {
              ""
            },
            ...
            tilgang_utvidet = if (!is.null(input$tilgang_utvidet2)) {
              input$tilgang_utvidet2
            } else {
              ""
            },
            minPRS = as.numeric(input$PRS2[1]),
            maxPRS = as.numeric(input$PRS2[2]),
            ASA = if (!is.null(input$ASA2)) {
              input$ASA2
            } else {
              ""
            },
            modGlasgow = if (!is.null(input$modGlasgow2)) {
              input$modGlasgow2
            } else {
              ""
            },
            whoEcog = if (!is.null(input$whoEcog2)) {
              input$whoEcog2
            } else {
              ""
            },
            forbehandling = if (!is.null(input$forbehandling2)) {
              input$forbehandling2
            } else {
              ""
            },
            malign = as.numeric(input$malign2),
            kun_ferdigstilte = input$kun_ferdigstilte,
            icd = if (!is.null(input$icd_verdi2)) {
              input$icd_verdi2
            } else {
              ""
            },
            accordion = if (!is.null(input$accordion2)) {
              input$accordion2
            } else {
              ""
            }
        ...
        shiny::isolate(if (!is.null(input$enhetsUtvalg2)) {
          if (input$enhetsUtvalg2 == 2) {
            Utvalg2data <- Utvalg2data[which(Utvalg2data$AvdRESH == reshID()), ]
          }
        })
        ...
        if (input$ekskluder_felles) {
          Samlet <- Samlet[!(Samlet$PasientID %in% intersect(
            Utvalg1data$PasientID,
      ...
      output$Figur_surv <- renderPlot({
        ...
        survminer::ggsurvplot(overlevdata$fit1,
          data = overlevdata$Samlet,
      ...
      output$utvalg <- renderUI({
        ...
        tagList(
          ...
          shiny::isolate(h5(if (!is.null(input$enhetsUtvalg)) {
            if (input$enhetsUtvalg == 2) {
              HTML(paste0("Avdeling: ", paste(unique(utvlgdata$Utvalg1$Sykehusnavn), collapse = ", ")))
            }
          })),
          br(),
          br(),
          ...
          shiny::isolate(h5(if (!is.null(input$enhetsUtvalg2)) {
            if (input$enhetsUtvalg2 == 2) {
              HTML(paste0("Avdeling: ", paste(unique(utvlgdata$Utvalg2$Sykehusnavn), collapse = ", ")))
            }
          })),
          br(),
          br(),
          ...
          if (input$ekskluder_felles) {
            h5(paste0(
              length(setdiff(utvlgdata$Utvalg1$PasientID, utvlgdata$Samlet$PasientID[utvlgdata$Samlet$Utvalg == 1])),
      ...
      output$utvalg2 <- renderUI({
        ...
        tagList(
          ...
          if (input$ekskluder_felles) {
            h5(paste0(
              length(intersect(utvlgdata$Utvalg1$ForlopsID, utvlgdata$Utvalg2$ForlopsID)),
      ...
      output$lastNedBilde <- downloadHandler(
        ...
        content = function(file) {
          ...
          survp <- survminer::ggsurvplot(
            overlevdata$fit1,
            data = overlevdata$Samlet,
            title = input$tittel, pval = TRUE,
            conf.int = input$inkl_konf, fun = "pct",
            risk.table = TRUE, legend = "bottom"
          )
      ...
      output$lastNedBilde2 <- downloadHandler(
        ...
        content = function(file) {
          overlevdata <- calc_overlevelse_v2()
          survp <- survminer::ggsurvplot(overlevdata$fit1,
            data = overlevdata$Samlet, title = input$tittel, pval = TRUE,
