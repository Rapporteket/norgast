...
norgastBeregnIndikator <- function(RegData, ind_id) {
  map_resh_orgnr <- data.frame(
    ...
    orgnr_sh = c(
      974733013, 974631407, 974557746, 974632535,
      974795787, 974705788, 974633574, 974795639,
      974724960, 974795361, 993467049, 974631326,
      974749025, 974706490, 974703300, 974633752,
      874632562, 974743272, 974795515, 974116804,
  ...
  map_resh_orgnr$Sykehus <- RegData$Sykehusnavn[match(
    map_resh_orgnr$resh,
    RegData$AvdRESH
  )]
  ...
  if (ind_id == "norgast_vekt_reg") {
    Indikator <- RegData %>%
      dplyr::filter(
        Op_gr %in% 1:8, # Kun obligatoriske
        OppfStatus == 1 | is.na(OppfStatus), # Kun ferdige
        Hastegrad_hybrid == 1,
        Malign == 1
      ) %>%
      dplyr::mutate(
        var = ifelse(is.na(VekttapProsent), 0, 1),
        context = "caregiver",
        denominator = 1,
  ...
  if (ind_id == "norgast_aktivkontroll") {
    Indikator <- RegData %>%
      ...
      dplyr::mutate(
        var = dplyr::case_when(
          TelefonKontroll == 1 | FysiskKontroll == 1 ~ 1,
  ...
  if (ind_id == "norgast_lekkasje_tykktarm") {
    ...
    utvalgTxt <- c(
      "Operasjonsgruppe: Kolonreseksjoner", "WHO ECOG score: 0, 1",
  ...
  if (ind_id == "norgast_lekkasje_endetarm") {
    ...
    tittel <- "Andel anastomoselekkasjer, ny anastomose"
    utvalgTxt <- c(
      "Operasjonsgruppe: Rektumreseksjoner", "WHO ECOG score: 0, 1",
      "Hastegrad: Elektiv", "Diagnose: Malign"
    )
  }
  ...
  if (ind_id == "anastomoselekk_ventrikkel") {
    ...
    tittel <- "Andel anastomoselekkasjer, ny anastomose"
    utvalgTxt <- c(
      "Operasjonsgruppe: Ventrikkelreseksjoner", "Hastegrad: Elektiv",
      "Diagnose: Malign"
    )
  }
  ...
  if (ind_id == "relap_ventrikkel") {
    ...
    tittel <- "Andel relaparotomi/laparoskopi"
    utvalgTxt <- c(
      "Operasjonsgruppe: Ventrikkelreseksjoner",
      "Hastegrad: Elektiv"
    )
  }
  ...
  if (ind_id == "relap_whipple") {
    ...
    tittel <- "Andel relaparotomi/laparoskopi"
    utvalgTxt <- c(
      "Operasjonsgruppe: Whipples operasjon",
      "Hastegrad: Elektiv"
    )
  }
  ...
  if (ind_id == "kikkhullsteknikk_distal") {
    ...
    tittel <- "Andel laparoskopi (konverterte inngrep inkludert)"
    utvalgTxt <- c(
      "Operasjonsgruppe: Distale pankreasreseksjoner",
      "Hastegrad: Elektiv"
    )
  }
  ...
  if (ind_id == "relap_lever") {
    ...
    tittel <- "Andel relaparotomi/laparoskopi"
    utvalgTxt <- c(
      "Operasjonsgruppe: Leverreseksjoner",
      "Hastegrad: Elektiv"
    )
  }
  ...
  tabell <- merge(tabell2, tabell1,
    by = "Sykehus", suffixes = c("", "_saml"),
    all.y = T
  )
  ...
  utdata <- list(
    Indikator = Indikator, tittel = tittel, utvalgTxt = utvalgTxt,
...
norgastPlotIndikator <- function(
  ...
  if (maal > minstekrav & !is.na(maal) & !is.na(minstekrav)) {
    rect(
      xleft = minstekrav, ybottom = 1, xright = maal, ytop = max(ypos) - 1.6,
      col = fargerMaalNiva[2], border = NA
    )
    rect(
      xleft = maal, ybottom = 1, xright = min(xmax, 100), ytop = max(ypos) - 1.6,
      col = fargerMaalNiva[1], border = NA
    )
  }
  if (maal < minstekrav & !is.na(maal) & !is.na(minstekrav)) {
    rect(
      xleft = maal, ybottom = 1, xright = minstekrav, ytop = max(ypos) - 1.6,
      col = fargerMaalNiva[2], border = NA
    )
    rect(
      xleft = 0, ybottom = 1, xright = maal, ytop = max(ypos) - 1.6,
      col = fargerMaalNiva[1], border = NA
    )
  }
