...
NorgastHentData <- function(datoFra = "2014-01-01", datoTil = "2099-01-01") {
  ...
  shiny::withProgress(message = "Loading data", value = 0, {
    ...
    #   registryName, "SELECT * FROM registration")
    # readmission <- rapbase::loadRegData(
    #   registryName, "SELECT * FROM readmission")
    # centre <- rapbase::loadRegData(
    #   registryName, "SELECT * FROM centre")
    # user <- rapbase::loadRegData(
    #   registryName, "SELECT * FROM user")
    # centreattribute <- rapbase::loadRegData(
    #   registryName, "SELECT * FROM centreattribute") |>
    #   dplyr::filter(ATTRIBUTENAME == "FRIENDLYNAME")
    # mce <- rapbase::loadRegData(
    #   registryName, "SELECT * FROM mce")
    # patient <- rapbase::loadRegData(
    #   registryName, "SELECT * FROM patient")


    dbList <- rapbase::rapOpenDbConnection("norgast", "mysql")

    shiny::incProgress(0.1, detail = "Laster operasjonsdata")
    registration <- DBI::dbGetQuery(
      conn = dbList$con,
      statement = "SELECT * FROM registration"
    )
    shiny::incProgress(0.1, detail = "Laster oppfølgingsdata")
    readmission <- DBI::dbGetQuery(
      conn = dbList$con,
      statement = "SELECT * FROM readmission"
    )
    shiny::incProgress(0.1, detail = "Laster senterdata")
    centre <- DBI::dbGetQuery(
      conn = dbList$con,
      statement = "SELECT * FROM centre"
    )
    centreattribute <- DBI::dbGetQuery(
      conn = dbList$con,
      statement = "SELECT * FROM centreattribute"
    ) |>
      dplyr::filter(ATTRIBUTENAME == "FRIENDLYNAME")
    shiny::incProgress(0.1, detail = "Laster brukerdata")
    user <- DBI::dbGetQuery(
      conn = dbList$con,
      statement = "SELECT * FROM user"
    )
    shiny::incProgress(0.1, detail = "Laster forløpsdata")
    mce <- DBI::dbGetQuery(
      conn = dbList$con,
      statement = "SELECT * FROM mce"
    )
    shiny::incProgress(0.1, detail = "Laster pasientdata")
    patient <- DBI::dbGetQuery(
      conn = dbList$con,
    ...
    skjemaoversikt <- registration |>
      ...
      dplyr::select(
        Skjemanavn, STATUS, MCEID, CREATEDBY, OPERATION_DATE,
      ...
      dplyr::mutate(
        ...
        OpprettetAv = paste0(
          user$FIRSTNAME[match(CREATEDBY, user$ID)], " ",
          user$LASTNAME[match(CREATEDBY, user$ID)]
        ),
        SistLagretAv = paste0(
          user$FIRSTNAME[match(UPDATEDBY, user$ID)], " ",
      ...
      dplyr::select(
        Skjemanavn, SkjemaStatus, ForlopsID, OpprettetAv,
      ...
      dplyr::bind_rows(
        readmission |>
          ...
          dplyr::select(
            Skjemanavn, STATUS, MCEID, CREATEDBY,
            UPDATEDBY, TSCREATED, TSUPDATED, CENTREID
          ) |>
          dplyr::mutate(
            Sykehusnavn = centreattribute$ATTRIBUTEVALUE[
    ...
    varnavn_patient <-
      setNames(
        varnavn_kobl$var_navn[varnavn_kobl$tabell == "patient"],
        varnavn_kobl$rapporteket[varnavn_kobl$tabell == "patient"]
      )
    varnavn_registration <-
      setNames(
        varnavn_kobl$var_navn[varnavn_kobl$tabell == "registration"],
        varnavn_kobl$rapporteket[varnavn_kobl$tabell == "registration"]
      )
    varnavn_centre <-
      setNames(
        varnavn_kobl$var_navn[varnavn_kobl$tabell == "centre"],
        varnavn_kobl$rapporteket[varnavn_kobl$tabell == "centre"]
      )
    varnavn_readmission <-
      setNames(
        varnavn_kobl$var_navn[varnavn_kobl$tabell == "readmission"],
    ...
    RegData <- allevarnum |>
      ...
      merge(
        skjemaoversikt |> dplyr::filter(Skjemanavn == "Registrering") |>
      ...
      dplyr::rename(
        OppfOpprettetAv = OpprettetAv_oppf,
    ...
    rm(list = c(
      "centre", "centreattribute", "mce",
      "patient", "user",
      "varnavn_kobl", "readmission",
      "registration", "allevarnum"
    ))
    # tictoc::toc()
  })
  return(list(
...
#' Provide global dataframe for NORGAST
#'
#' Provides NORGAST data from local database
#'
#' @inheritParams FigAndeler
#'
#' @return RegData data frame
#' @export
#'
NorgastHentDataLokal <- function(datoFra = "2014-01-01", datoTil = "2099-01-01") {
  if (Sys.getenv("R_RAP_INSTANCE") %in% c("QAC", "PRODUCTIONC")) {
    registryName <- "data"
  } else {
    registryName <- "norgast"
  ...
  rm(list = c(
    "centre", "centreattribute", "mce", "patient", "user",
...
NorgastHentRegData <- function(datoFra = "2014-01-01", datoTil = "2099-01-01") {
  ...
  if (rapbase::isRapContext()) {
    ...
    query2 <- paste0(
      "SELECT * FROM forlopsoversikt WHERE HovedDato >= \'",
  ...
  RegData <- merge(allevarnum,
    ...
    merge(
      skjemaoversikt %>% dplyr::filter(SkjemaRekkeflg == 5) %>%
        dplyr::select(ForlopsID, OpprettetAv, SistLagretAv),
      by = "ForlopsID", all.x = TRUE
    ) %>%
    merge(
      skjemaoversikt %>% dplyr::filter(SkjemaRekkeflg == 10) %>%
        dplyr::select(ForlopsID, OpprettetAv, SistLagretAv),
      suffixes = c("", "_oppf"), by = "ForlopsID", all.x = TRUE
    ) %>%
    dplyr::rename(
