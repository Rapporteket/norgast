...
NorgastFigAndelerGrVar <- function(RegData = 0, valgtVar = "", datoFra = "2014-01-01", datoTil = "2050-12-31",
  ...
  if (valgtShus[1] != "") {
    RegData <- RegData[which(RegData$AvdRESH %in% as.numeric(valgtShus)), ]
  }
  ...
  if (max(Ngr) < Ngrense) { # Dvs. hvis ALLE er mindre enn grensa.
    ...
    pos <- barplot(as.numeric(AndelerGrSort),
      horiz = T, border = NA, col = farger[3], # main=tittel,
    ...
    if (inkl_konf == 1) {
      ...
      legend("topright",
        xpd = TRUE, xjust = 0, yjust = 0, pch = c(NA, 15), pt.cex = 2, cex = 0.9, # y=ytopp+0.5,
      ...
      legend("topright",
        xjust = 1, cex = 1, lwd = 2, col = farger[2],
    ...
    if (inkl_konf != 1) {
      text(
        ...
        las = 1, cex = 0.8, adj = 0, col = farger[1]
      ) # Andeler, hvert sykehus
    }

    barplot(as.numeric(AndelerGrSort),
      horiz = T, border = NA, col = farger[3], xlim = c(0, xmax), add = TRUE,
      font.main = 1, las = 1
    )
    ...
    if (inkl_konf == 1) {
      arrows(
        x0 = KI[1, (length(pos) - AntGr + 1):length(pos)], y0 = posKI, x1 = KI[2, (length(pos) - AntGr + 1):length(pos)], y1 = posKI,
    ...
    utData <- list(
      tittel = tittel, utvalgTxt = utvalgTxt, Andeler = AndelerGrSort, AndelHele = AndelHele,
