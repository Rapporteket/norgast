  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$valgtVar_gjsn_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("valgtVar_gjsn"), label = "Velg variabel",
      ...
      output$valgtVar_andel_stabel_ui <- renderUI({
        ns <- session$ns
        selectInput(
          inputId = ns("valgtVar_andel_stabel"), label = "Velg variabel",
      ...
      output$Tabell_sykehus_andel <- function() {
        ...
        Tabell <- dplyr::tibble(
          Avdeling = names(utdata$Nvar), Antall = utdata$Nvar,
      ...
      output$Tabell_sykehus_andel_stabel <- function() {
        ...
        Tabell %>%
          knitr::kable("html", digits = c(rep(0, dim(Tabell)[2] / 2 + 1), rep(1, dim(Tabell)[2] / 2 - 1))) %>%
          kableExtra::kable_styling("hover", full_width = F) %>%
      ...
      output$fig_gjsn_grvar <- renderPlot(
        {
          norgast::NorgastFigGjsnGrVar(
            ...
            valgtVar = if (!is.null(input$valgtVar_gjsn)) {
              input$valgtVar_gjsn
            } else {
              "BMI"
            },
