...
NorgastFigGjsnGrVar <- function(RegData = 0, valgtVar = "Alder", datoFra = "2014-01-01", datoTil = "2050-12-31",
                                ...
                                preprosess = F, malign = 99, Ngrense = 30, hastegrad = 99, hastegrad_hybrid = 99,
  ...
  if (max(Ngr) < Ngrense) { # Dvs. hvis ALLE er mindre enn grensa.
    ...
    barplot(Midt,
      ...
      font.main = 1, xlab = xaksetxt, las = 1
    ) # xlim=c(0,ymax), #, cex.names=0.5
    ...
    text(
      x = max(strwidth(soyletxt, units = "user", cex = cexSoyletxt)), y = pos + 0.1,
    ...
    arrows(
      x0 = Midt[-indGrUtPlot] * 0.999, y0 = posKI, x1 = KIopp[-indGrUtPlot], y1 = posKI,
      length = 0.5 / max(pos), code = 2, angle = 90, lwd = 1.5, col = farger[1]
    )
    arrows(
      x0 = Midt[-indGrUtPlot] * 1.001, y0 = posKI, x1 = KIned[-indGrUtPlot], y1 = posKI,
      length = 0.5 / max(pos), code = 2, angle = 90, lwd = 1.5, col = farger[1]
    )
    par("fig" = c(0, 1, 0, 1))
    if (outfile != "") {
