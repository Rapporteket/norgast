...
FigAndeler <- function(RegData = 0, valgtVar = "Alder", datoFra = "2014-01-01", datoTil = "2050-12-31",
  ...
  if (NHoved < 5 | (Nrest < 5 & enhetsUtvalg == 1)) {
    ...
    if (stabel) {
      ...
      names(Andel)[1] <- shtxt
      Andel <- as.matrix(Andel)
      tLeg <- "Kategorier"
      if (length(grtxt) == 2) {
        farger <- farger[c(2, 4)]
      }
      if (length(grtxt) == 3) {
        farger <- farger[c(1, 3, 4)]
      }
      ...
      legend(2.65, 99, rev(grtxt),
        bty = "n", ncol = 1, cex = cexgr, xjust = 0, fill = farger,
...
NorgastBeregnAndeler <- function(
  ...
  utData <- list(
    tittel = PlotParams$tittel, utvalgTxt = utvalgTxt, Andeler = Andeler,
