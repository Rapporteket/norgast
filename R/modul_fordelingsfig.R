  ...
  shiny::sidebarLayout(
    shiny::sidebarPanel(
      ...
      selectInput(
        ...
        choices = c(
          "Hele landet" = 0, "Egen avd. mot landet forøvrig" = 1,
          "Egen avd." = 2
        )
      ),
      ...
      selectInput(
        inputId = ns("malign"), label = "Diagnose", choices =
          c("Ikke valgt" = 99, "Malign" = 1, "Benign" = 0)
      ),
      uiOutput(outputId = ns("icd")),
      selectInput(
        inputId = ns("ny_stomi"), label = "Ny stomi",
        ...
        if (!is.null(input$op_gruppe)) {
          selectInput(
            ...
            choices = if (!is.null(input$op_gruppe)) {
              setNames(
                substr(sort(unique(RegData$Hovedoperasjon[
